#!/usr/bin/env python3
"""
Migration Orchestration Script - PRODUCTION READY ✅
Manages the complete Parquet to Supabase migration process

🎉 MAJOR TABLES COMPLETED (August 2025):
✅ companies (46,112 records) - Latest CVR enrichment data
✅ species (37 records) - CHR reference data
✅ production_sites (20,246 records) - CHR properties with CVR mapping
✅ field_yearly_data (2,895,145 records) - All years (2021-2025) FVM marker data
✅ field_boundaries (595,038 records) - PostGIS geometry with coordinate fixes
✅ pesticide_applications (4,693,176 records) - Historical data (2022-2024) with BMD + proximity

🔄 REMAINING TABLES (Post-Holiday Implementation):
✅ yearly_financials - CVR financial statements (completed)
✅ company_leadership - Company executives/board (completed)
✅ company_owners - Ownership structure (completed)
✅ antibiotic_usage - Veterinary antibiotic usage (completed)
⏳ animal_capacity_log - Animal capacity over time (needs implementation)
⏳ vet_events - Veterinary treatments (needs implementation)
⏳ animal_transports - Animal movement records (needs implementation)
⏳ incidents - Work safety incidents (needs implementation)
✅ worker_yearly_summary - Work injury statistics (ready in orchestrator)
✅ visa_yearly_counts - Work permit data (ready in orchestrator)

Usage:
    python run_migration.py --environment production --phase core  # Run completed migrations
    python run_migration.py --environment production --phase financial  # Future implementation
"""
...
from scripts.migration.parquet_to_supabase_etl import MigrationConfig, SupabaseMigrationPipeline
...
class MigrationOrchestrator:
    ...
    def __init__(self, env_config: EnvironmentConfig):
        ...
        self.migration_phases = {
            ...
            "animal": {
                ...
                "required": False,
                "tables": ["animal_capacity_log", "animal_transports", "vet_events", "antibiotic_usage"],
            },
            "worker": {
                "description": "Load worker and safety data",
                "function": self._run_worker_data_migration,
                "required": False,
                "tables": ["employee_monthly_counts", "visa_yearly_counts", "worker_yearly_summary", "incidents"],
            },
            "computed": {
    ...
    async def _run_reference_data_migration(self, tables: Optional[List[str]], dry_run: bool):
        """Load reference data (species, etc.) - ✅ PRODUCTION READY"""
    ...
    async def _run_core_data_migration(self, tables: Optional[List[str]], dry_run: bool):
        """Load core entity data - ✅ PRODUCTION READY"""
    ...
    async def _run_field_data_migration(self, tables: Optional[List[str]], dry_run: bool):
        """Load field and environmental data - ✅ field_yearly_data PRODUCTION READY, others need implementation"""
    ...
    async def _run_pesticide_data_migration(self, tables: Optional[List[str]], dry_run: bool):
        """Load pesticide application data - ✅ PRODUCTION READY"""
    ...
    async def _run_animal_data_migration(self, tables: Optional[List[str]], dry_run: bool):
        ...
        animal_tables = ["animal_capacity_log", "animal_transports", "vet_events", "antibiotic_usage"]
    ...
    async def _run_worker_data_migration(self, tables: Optional[List[str]], dry_run: bool):
        ...
        worker_tables = ["employee_monthly_counts", "visa_yearly_counts", "worker_yearly_summary", "incidents"]
    ...
    async def _run_computed_data_generation(self, tables: Optional[List[str]], dry_run: bool):
        ...
        config = MigrationConfig(
            supabase_host=self.env_config.supabase_host, supabase_password=self.env_config.supabase_password
        )
...
def main():
    ...
    parser.add_argument(
        "--environment", required=True, choices=["local", "staging", "production"], help="Target environment"
    )
    parser.add_argument(
        "--phase",
        nargs="+",
    ...
    try:
        ...
        failed_phases = [p for p, r in results.items() if r["status"] == "failed"]
        if failed_phases:
            logger.error(f"Migration failed with {len(failed_phases)} failed phases")
            sys.exit(1)
        else:
