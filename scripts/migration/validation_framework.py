...
class MigrationValidator:
    ...
    def __init__(self, supabase_host: str, supabase_password: str, gcs_bucket: str = "gs://landbrugsdata-raw-data"):
        ...
        self.table_validations = {
            ...
            "production_sites": {
                ...
                "data_quality_checks": [
                    ...
                    (
                        "valid_coordinates",
                        lambda df: df["location_geom"].notna().sum() > len(df) * 0.8,
            ...
            "yearly_financials": {
                ...
                "data_quality_checks": [
                    ...
                    (
                        "reasonable_values",
                        lambda df: df["total_assets"].abs().max() < 1e12 if "total_assets" in df.columns else True,
                    ),
                ],
            ...
            "pesticide_applications": {
                ...
                "data_quality_checks": [
                    ...
                    (
                        "proximity_reasonable",
                        lambda df: (df["proximity_water_m"] >= 0).all() if "proximity_water_m" in df.columns else True,
                    ),
                ],
    ...
    async def run_full_validation(self) -> ValidationSuite:
        ...
        try:
            ...
            suite.add_result(
                ValidationResult(
                    check_name="validation_framework_error", status="error", message=f"Validation framework error: {e}"
                )
            )
    ...
    async def _validate_schema(self) -> List[ValidationResult]:
        ...
        try:
            ...
            if missing_tables:
                ...
                results.append(
                    ValidationResult(check_name="all_tables_exist", status="pass", message="All expected tables exist")
                )
            ...
            results.append(
                ValidationResult(
                    check_name="schema_validation_error", status="error", message=f"Schema validation failed: {e}"
                )
            )
    ...
    async def _validate_data_integrity(self) -> List[ValidationResult]:
        ...
        try:
            ...
            results.append(
                ValidationResult(
                    check_name="data_integrity_error", status="error", message=f"Data integrity validation failed: {e}"
                )
            )
    ...
    async def _validate_data_quality(self) -> List[ValidationResult]:
        ...
        try:
            ...
            results.append(
                ValidationResult(
                    check_name="data_quality_error", status="error", message=f"Data quality validation failed: {e}"
                )
            )
    ...
    async def _validate_business_rules(self) -> List[ValidationResult]:
        ...
        try:
            ...
            results.append(
                ValidationResult(
                    check_name="business_rules_error", status="error", message=f"Business rules validation failed: {e}"
                )
            )
    ...
    async def _validate_performance(self) -> List[ValidationResult]:
        ...
        try:
            ...
            performance_tests = [
                ...
                (
                    "field_boundaries_spatial",
                    "SELECT COUNT(*) FROM field_boundaries WHERE ST_Area(geom) > 0",
                    1000,
                ),  # 1s threshold
                (
                    "pesticide_applications_year",
                    "SELECT COUNT(*) FROM pesticide_applications WHERE year = 2024",
                    500,
                ),  # 500ms threshold
                (
                    "production_sites_company",
                    "SELECT * FROM production_sites WHERE company_id = (SELECT id FROM companies LIMIT 1)",
                    200,
                ),  # 200ms threshold
            ]
    ...
    async def _validate_data_completeness(self) -> List[ValidationResult]:
        ...
        try:
            ...
            if empty_tables:
                ...
                results.append(
                    ValidationResult(
                        check_name="data_completeness_no_empty_tables", status="pass", message="No empty tables found"
                    )
                )
...
async def main():
    ...
    # Run validation
    validator = MigrationValidator(supabase_host=args.host, supabase_password=args.password)
