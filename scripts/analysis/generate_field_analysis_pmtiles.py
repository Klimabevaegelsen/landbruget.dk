...
def get_all_properties(conn: duckdb.DuckDBPyConnection) -> List[str]:
    ...
    logger.info(f"   Tippecanoe will automatically optimize for different zoom levels")
...
def generate_pmtiles_with_tippecanoe(
    geojson_file: str,
    output_pmtiles: str,
...
def generate_metadata(
    ...
    metadata = {
        ...
        "description": "Comprehensive field-level analysis of Danish agriculture including pesticide usage, environmental areas, soil types, and building proximity",
        "version": "1.0.0",
        "source": {
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate PMTiles from comprehensive field analysis data"
    )
    parser.add_argument(
        "--input", 
        required=True,
        help="Input parquet file from recreate_original_csv_structure.py"
    )
    parser.add_argument(
        "--output", 
        required=True,
    ...
    parser.add_argument(
        ...
        type=int, 
        default=10,
        help="Base zoom level for optimal detail (default: 10)"
    )
    parser.add_argument(
        "--temp-dir",
    ...
    try:
        ...
        success = generate_pmtiles_with_tippecanoe(
            str(geojson_file),
            args.output,
            zoom_config
        )
