...
export function BlockTable({ grid }: { grid: BaseDataGrid }) {
  const columns: ColumnDef<Record<string, string | number | boolean>>[] = grid.columns.map(
    (col) => ({
      accessorKey: col.key,
      header: ({ column }) => {
        return (
  ...
  return (
    <div className="">
      <DynamicDataTable columns={columns} data={grid.rows} filterable={grid.allowFiltering} />
    </div>
  );
}
