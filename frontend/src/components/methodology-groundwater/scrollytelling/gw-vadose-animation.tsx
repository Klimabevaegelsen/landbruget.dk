...
function getConfig(variant: 'bentazon' | 'mcpa') {
  if (variant === 'bentazon') {
    return { duration: 2, lineHeight: 120, label: '6 m', color: '#ef4444' };
  }
  return { duration: 5, lineHeight: 200, label: '19,3 m', color: '#f59e0b' };
}
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="pointer-events-none absolute top-1/4 left-1/2 z-10 -translate-x-1/2"
