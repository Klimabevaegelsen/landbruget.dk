  ...
  persist(
    (set, get) => ({
      ...
      setMetadataError: (error) =>
        set({ metadataError: error, metadataLoading: false }),

      // Tile loading actions
      setTileLoading: (key, url) =>
        set((state) => ({
          loadedTiles: {
            ...state.loadedTiles,
            [key]: {
      ...
      isYearResolutionAvailable: (year, resolution) => {
        ...
        return (
          metadata.years.includes(year) &&
          metadata.resolutions.includes(resolution)
        );
      },
