    ...
    map.on("load", () => {
      ...
      const geojson: GeoJSON.FeatureCollection = {
        ...
        features: validPoints.map((row, i) => ({
          ...
          properties: Object.fromEntries(Object.entries(row).map(([k, v]) => [k, String(v ?? "")])),
        })),
      };
      ...
      map.addLayer({
        ...
        type: "circle",
        source: "points",
        filter: validPoints.length > 200 ? ["!", ["has", "point_count"]] : ["all"],
        paint: {
          "circle-color": "#2d5a3d",
          "circle-radius": 6,
      ...
      map.on("click", "unclustered-point", (e) => {
        ...
        const html = keys.map((k) => `<b>${k}:</b> ${props[k] ?? ""}`).join("<br/>");
