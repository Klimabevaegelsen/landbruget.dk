...
function buildCatalogue(datasets: Dataset[]): string {
  return datasets
    .map(
      (ds) =>
        `- ${ds.name}: ${ds.displayName}. ${ds.description} (${ds.rowCount.toLocaleString()} rows)`,
    )
    .join("\n");
}
        ...
        ? ds.schema
            .map((c) => {
              ...
              if (stats?.isEmpty) {
                ...
                if (stats?.sampleValues?.length)
                  line += ` [e.g. ${stats.sampleValues.slice(0, 3).join(", ")}]`;
                if (
                  stats?.min !== undefined &&
                  stats?.max !== undefined &&
                  stats.min &&
...
export async function POST(request: NextRequest) {
  try {
    ...
    const selectionResult =
      await selectorModel.generateContent(`You are a schema linker for a Danish agricultural database.
