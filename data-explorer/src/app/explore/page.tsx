  ...
  const generateVisualization = useCallback(
    async (
      ...
      try {
        ...
        if (!response.ok) throw new Error(data.error || "Failed to generate visualization");
        setUISpec(injectData(data.spec, results));
      } catch (err) {
        setUIError(err instanceof Error ? err.message : "Failed to generate visualization");
      } finally {
    ...
    <div className="flex min-h-screen flex-col" style={{ background: "var(--background)" }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          ...
          <div className="h-0.5 flex-shrink-0" style={{ background: "var(--primary)" }} />
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="mx-auto max-w-3xl space-y-8 px-6 pt-8 pb-16">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs" style={{ color: "var(--muted-foreground)" }}>
                    {new Intl.NumberFormat("da-DK").format(selectedDataset.rowCount)} rows
                    {" · "}
            ...
            <div>
              {(viewMode === "smart" || viewMode === "both") && hasSmartView && (
                <SmartResults spec={uiSpec} loading={isGeneratingUI} error={uiError} />
              )}
          ...
          {/* Slim footer */}
          <footer className="border-t px-6 py-3" style={{ borderColor: "var(--border)" }}>
            <div
