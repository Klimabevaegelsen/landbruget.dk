...
async function getLandUseDetails(
  supabase: any,
  params: MunicipalityDetailsRequest
): Promise<MunicipalityDetailsResponse> {
  ...
  const municipalityFieldUuids = new Set(
    municipalityFields.map((f: any) => f.field_uuid)
  );
  ...
  const companies = Object.values(companyAggregates)
    .map((company: any) => ({
      ...company,
      organic_percentage:
        company.total_area > 0
          ? (company.organic_area / company.total_area) * 100
          : 0,
    }))
    .sort((a: any, b: any) => b.total_area - a.total_area)
    .slice(0, params.limit);

  const totalMunicipalityValue = companies.reduce(
    (sum: number, c: any) => sum + c.total_area,
    0
  );
  ...
  return {
    ...
    companies: companies.map((company: any, index: number) => ({
      ...
      percentage_of_municipality:
        totalMunicipalityValue > 0
          ? (company.total_area / totalMunicipalityValue) * 100
          : 0,
...
async function getProductionDetails(
  ...
  const totalMunicipalityValue = data.reduce(
    (sum: number, site: any) => sum + (site.capacity || 0),
    0
  );
  ...
  return {
    ...
    companies: data.map((site: any, index: number) => ({
      ...
      percentage_of_municipality:
        totalMunicipalityValue > 0
          ? ((site.capacity || 0) / totalMunicipalityValue) * 100
          : 0,
      rank_in_municipality: index + 1,
      additional_data: {
...
async function getPesticideBurdenDetails(
  supabase: any,
  params: MunicipalityDetailsRequest
): Promise<MunicipalityDetailsResponse> {
  ...
  const totalMunicipalityValue = companies.reduce(
    (sum: number, c: any) => sum + c.total_burden,
    0
  );
  ...
  return {
    ...
    companies: companies.map((company: any, index: number) => ({
      ...
      percentage_of_municipality:
        totalMunicipalityValue > 0
          ? (company.total_burden / totalMunicipalityValue) * 100
          : 0,
      rank_in_municipality: index + 1,
      additional_data: {
...
async function getPFASDetails(
  ...
  const totalMunicipalityValue = companies.reduce(
    (sum: number, c: any) => sum + c.pfas_burden,
    0
  );
  ...
  return {
    ...
    companies: companies.map((company: any, index: number) => ({
      ...
      percentage_of_municipality:
        totalMunicipalityValue > 0
          ? (company.pfas_burden / totalMunicipalityValue) * 100
          : 0,
      rank_in_municipality: index + 1,
      additional_data: {
...
async function getGlyphosateDetails(
  supabase: any,
  params: MunicipalityDetailsRequest
): Promise<MunicipalityDetailsResponse> {
  ...
  const totalMunicipalityValue = companies.reduce(
    (sum: number, c: any) => sum + c.glyphosate_burden,
    0
  );
  ...
  return {
    ...
    companies: companies.map((company: any, index: number) => ({
      ...
      percentage_of_municipality:
        totalMunicipalityValue > 0
          ? (company.glyphosate_burden / totalMunicipalityValue) * 100
          : 0,
      rank_in_municipality: index + 1,
      additional_data: {
...
async function getAntibioticDetails(
  supabase: any,
  params: MunicipalityDetailsRequest
): Promise<MunicipalityDetailsResponse> {
  ...
  const totalMunicipalityValue = data.reduce(
    (sum: number, site: any) => sum + (site.antibiotics_ddd || 0),
    0
  );
  ...
  return {
    ...
    companies: data.map((site: any, index: number) => ({
      ...
      percentage_of_municipality:
        totalMunicipalityValue > 0
          ? ((site.antibiotics_ddd || 0) / totalMunicipalityValue) * 100
          : 0,
      rank_in_municipality: index + 1,
      additional_data: {
        chr_number: site.chr,
...
async function getEnvironmentalDetails(
  supabase: any,
  params: MunicipalityDetailsRequest
): Promise<MunicipalityDetailsResponse> {
  ...
  const companyAggregates = fieldData
    ...
    .reduce((acc: any, row: any) => {
      ...
      acc[companyId].total_n_leached +=
        (row.n_leached_kg || 0) * (row.area_ha || 0);
      acc[companyId].total_area += row.area_ha || 0;
      acc[companyId].field_count += 1;
      return acc;
    }, {});
  ...
  const companies = Object.values(companyAggregates)
    .map((company: any) => ({
      ...company,
      avg_n_leached_per_ha:
        company.total_area > 0
          ? company.total_n_leached / company.total_area
          : 0,
    }))
  ...
  const totalMunicipalityValue = companies.reduce(
    (sum: number, c: any) => sum + c.total_n_leached,
    0
  );
  ...
  return {
    ...
    companies: companies.map((company: any, index: number) => ({
      ...
      percentage_of_municipality:
        totalMunicipalityValue > 0
          ? (company.total_n_leached / totalMunicipalityValue) * 100
          : 0,
      rank_in_municipality: index + 1,
      additional_data: {
        field_count: company.field_count,
...
async function getOrganicFarmingDetails(
  supabase: any,
  params: MunicipalityDetailsRequest
): Promise<MunicipalityDetailsResponse> {
  ...
  const totalMunicipalityValue = companies.reduce(
    (sum: number, c: any) => sum + c.organic_area,
    0
  );
  ...
  return {
    ...
    companies: companies.map((company: any, index: number) => ({
      ...
      percentage_of_municipality:
        totalMunicipalityValue > 0
          ? (company.organic_area / totalMunicipalityValue) * 100
          : 0,
      rank_in_municipality: index + 1,
      additional_data: {
        field_count: company.field_count,
