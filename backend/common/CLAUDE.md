...
- `SharedDuckDBProcessor(db_path=":memory:", dataset_name="data")` — base class for all DuckDB ops
  ...
  - Methods: `table_exists()`, `create_table_from_parquet()`, `create_table_from_csv()`, `create_spatial_table()`, `create_table_from_gcs_parquet()`, `save_table_to_gcs_parquet()`, `get_row_count()`, `execute_query()`, `create_spatial_index()`
...
### `gcs/core.py` — `GCSDataAccess`
- Initializes gcsfs + DuckDB fsspec registration
- Two-tier cloud auth: native HMAC first, then gcsfs fallback
...
- Backward compat aliases: `get_gcs_filesystem = get_r2_filesystem`, `get_duckdb_with_gcs = get_duckdb_with_r2`
...
- Bucket resolution: `R2_BUCKET` → `GCS_BUCKET` → `"landbruget-data"`
- `CloudStorage(bucket_name)` — wraps GCSDataAccess for `save_json()`, `save_parquet()`, `read_json()`
...
- `mock_gcs_filesystem` — mocked cloud storage
...
- Module path is `common.gcs.core` NOT `common.gcs_utils` (legacy name in some docs)
- DuckDB connection MUST load spatial extension before geometry operations (auto-loaded by `SharedDuckDBProcessor`)
- `GCSDataAccess` import fails gracefully if `unified_pipeline` modules not available
