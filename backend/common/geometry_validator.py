"""
Shared geometry validation utilities for landbruget.dk pipelines.
...
def validate_and_transform_geometries_duckdb(
    ...
    try:
        ...
        if "VARCHAR" in geom_types:
            ...
            logger.info(f"{dataset_name}: Geometries are already spatial objects, skipping conversion")
        ...
        if invalid_count > 0:
            ...
            try:
                ...
                if still_invalid > 0:
                    logger.error(f"{dataset_name}: {still_invalid} geometries remain invalid, removing them")
                    conn.execute(f"""
        ...
        if initial_bounds:
            ...
            is_wgs84_lon_lat = 3 <= min_x <= 17 and 3 <= max_x <= 17 and 53 <= min_y <= 59 and 53 <= max_y <= 59
            is_wgs84_lat_lon = 53 <= min_x <= 59 and 53 <= max_x <= 59 and 3 <= min_y <= 17 and 3 <= max_y <= 17
            is_utm = 400000 <= min_x <= 900000 and 6000000 <= min_y <= 7000000

            if is_wgs84_lon_lat:
                logger.info(f"{dataset_name}: Data in WGS84 (LON, LAT) order - CORRECT for PostGIS EPSG:4326 standard")
        ...
        if not already_flipped_for_wgs84:
            ...
            try:
                ...
                if sample_wkt:
                    logger.info(
                        f"{dataset_name}: COORDINATE ORDER VERIFICATION - Found {len(sample_wkt)} sample centroids"
                    )
                    coord_pairs = []
                    for i, (wkt,) in enumerate(sample_wkt[:3]):
                        logger.debug(f"   Raw WKT {i + 1}: {wkt}")
                        if wkt and ("POINT(" in wkt or "POINT (" in wkt):
                            coords_str = wkt.replace("POINT(", "").replace("POINT (", "").replace(")", "").strip()
                            try:
                    ...
                    if coord_pairs:
                        ...
                        if (
                            ...
                            logger.warning(f"{dataset_name}: ALERT - Data stored as (LON, LAT) GIS convention")
                        else:
                            logger.warning(f"{dataset_name}: UNCLEAR - Coordinate order unclear")
            except Exception as e:
                logger.warning(f"{dataset_name}: Could not verify coordinate order: {e}")
        ...
        if invalid_wgs84 > 0:
            logger.warning(f"{dataset_name}: Found {invalid_wgs84} invalid geometries after WGS84 conversion")
            ...
            try:
                ...
                if final_invalid > 0:
                    logger.error(f"{dataset_name}: {final_invalid} geometries remain invalid, removing them")
                    conn.execute(f"""
                        DELETE FROM {table_name}
                        WHERE {geometry_column} IS NOT NULL AND NOT ST_IsValid({geometry_column})
                    """)
...
def verify_spatial_join_usage(conn: duckdb.DuckDBPyConnection, query: str) -> bool:
    ...
    try:
        ...
        if spatial_join_detected:
            ...
            logger.warning("SPATIAL_JOIN operator not used - query may use standard join")
            logger.warning(
...
def validate_and_normalize_to_utm(
    ...
    try:
        ...
        if bounds:
            ...
            logger.info(
                f"{dataset_name}: Coordinate bounds: X=[{min_x:.2f}, {max_x:.2f}], Y=[{min_y:.2f}, {max_y:.2f}]"
            )
            ...
            if is_utm:
                logger.info(f"{dataset_name}: Data already in EPSG:25832 (Danish UTM) - no transform needed")
        ...
        if final_bounds:
            ...
            if is_utm_final:
                ...
                logger.warning(
                    f"{dataset_name}: Output coordinates may not be EPSG:25832: "
