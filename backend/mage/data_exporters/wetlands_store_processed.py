...
def export_processed_wetlands(data, *args, **kwargs):
    ...
    config_profile = (
        "production" if os.getenv("MAGE_ENVIRONMENT") == "production" else "default"
    )
    ...
    try:
        ...
        if config_profile == "production":
            ...
            print(
                f"✅ Stored {validated_count:,} processed features to gs://{bucket_name}/{storage_path}"
            )
            storage_mode = "gcs"
        else:
            ...
            base_path = (
                storage_config.get("base_path")
                or os.getenv("MAGEAI_DATA_DIR")
                or "/home/src/mage_data"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_config = {"storage_path": "wetlands/test_processed.parquet"}

        result = export_processed_wetlands(test_data, config=test_config)
        print(
            f"Saved {result['feature_count']} features to {result['processed_data_path']} in {result['storage_mode']} mode"
        )
        print("Metadata:")
