...
def geo_validate_geometries(data, *args, **kwargs):
    ...
    if not input_path:
        raise ValueError(
            "No input path provided. Must provide 'input_path' or 'formatted_data_path'"
        )
    ...
    try:
        ...
        print(
            f"  • Success rate: {100.0 * metadata['stats']['valid_features'] / feature_count:.1f}%"
        )

        print("\n📐 Geometry Statistics:")
        print(
            f"  • Points per feature: {metadata['stats']['min_points']:,} to {metadata['stats']['max_points']:,}"
        )
        print(f"  • Average points: {metadata['stats']['avg_points']:.1f}")
        print(
            f"  • Area range: {metadata['stats']['min_area_m2']:.1f} to {metadata['stats']['max_area_m2']:.1f} m²"
        )
...
def test_output(data):
    ...
    assert "original_feature_count" in data, (
        "Output must contain original_feature_count"
    )
