...
def combine_batches(data, *args, **kwargs):
    ...
    try:
        ...
        for i, batch in enumerate(batches):
            ...
            batch_stats.append(
                {"batch_index": i + 1, "feature_count": len(batch_features)}
            )
        ...
        features_per_second = (
            feature_count / processing_time if processing_time > 0 else 0
        )
        ...
        for stat in batch_stats:
            print(
                f"  • Batch {stat['batch_index']}: {stat['feature_count']:,} features"
            )
        ...
        return {"geojson_data": combined_geojson, "metadata": metadata}
    ...
    if len(args) > 1:
        ...
        for i, batch_result in enumerate(args):
            assert isinstance(batch_result, dict), (
                f"Batch result {i} should be a dictionary"
            )
            assert "geojson_data" in batch_result, (
                f"Batch result {i} should contain geojson_data"
            )
            assert isinstance(batch_result["geojson_data"], dict), (
                f"Batch result {i} geojson_data should be a dictionary"
            )
            assert "features" in batch_result["geojson_data"], (
                f"Batch result {i} should contain features"
            )
        ...
        for i, batch_result in enumerate(data):
            ...
            assert "geojson_data" in batch_result, (
                f"Batch {i} should contain geojson_data"
            )
            assert isinstance(batch_result["geojson_data"], dict), (
                f"Batch {i} geojson_data should be a dictionary"
            )
            assert "features" in batch_result["geojson_data"], (
                f"Batch {i} should contain features"
            )
    else:
        ...
        assert isinstance(data["geojson_data"], dict), (
            "geojson_data should be a dictionary"
        )
        assert "type" in data["geojson_data"], "geojson_data should contain type"
        assert "features" in data["geojson_data"], (
            "geojson_data should contain features"
        )
        assert data["geojson_data"]["type"] == "FeatureCollection", (
            "geojson_data type should be FeatureCollection"
        )
        assert isinstance(data["geojson_data"]["features"], list), (
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Combined GeoJSON data length: {len(result['geojson_data']['features'])}"
        )
