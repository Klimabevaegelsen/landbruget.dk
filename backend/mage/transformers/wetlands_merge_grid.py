...
def process_chunk(chunk_gdf: gpd.GeoDataFrame, gridcode: int) -> gpd.GeoDataFrame:
    ...
    if filtered_df.crs is None:
        ...
        raise ValueError(
            f"Unexpected CRS EPSG:{filtered_df.crs.to_epsg()} - Pipeline requires EPSG:4326"
        )
    ...
    merged_df = filtered_df.dissolve(
        by="component_id",
        aggfunc={
            "gridcode": "first",
...
def create_spatial_chunks(
    gdf: gpd.GeoDataFrame, num_chunks: int = 4
...
def write_duckdb_compatible_geoparquet(gdf: gpd.GeoDataFrame, output_path: str) -> None:
    ...
    if df.crs is None:
        ...
        raise ValueError(
            f"Unexpected CRS EPSG:{df.crs.to_epsg()} - Pipeline requires EPSG:4326"
        )
    else:
    ...
    if b"geo" not in metadata:
        ...
        metadata_updated = {
            **{k.decode("utf8"): v.decode("utf8") for k, v in metadata.items()},
            "geo": json.dumps(geo_metadata),
        }

        # Convert keys back to bytes
        metadata_bytes = {
            k.encode("utf8"): v.encode("utf8") for k, v in metadata_updated.items()
        }
    ...
    print(
        f"💾 Wrote {len(df):,} features to GeoParquet file in DuckDB-compatible format (EPSG:4326)"
    )
...
def merge_grid(data_item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if gdf.crs is None:
        ...
        raise ValueError(
            f"Unexpected CRS EPSG:{gdf.crs.to_epsg()} - Pipeline requires EPSG:4326"
        )
    else:
        print("✅ Confirmed CRS is EPSG:4326")
    ...
    all_merged_dfs = []
    merge_stats = {
        12: {"original_features": 0, "merged_features": 0},
    ...
    for code in [12, 60]:
        ...
        if chunk_results:
            ...
            try:
                ...
                # Group by component and dissolve to merge adjacent polygons
                final_merged = merged_df.dissolve(
                    by="component_id",
                    aggfunc={
                        "gridcode": "first",
                ...
                print(
                    f"  🎯 Final merge for gridcode {code}: {len(merged_df)} features into {len(final_merged)}"
                )
    ...
    total_merged_features = sum(
        merge_stats[code]["merged_features"] for code in merge_stats
    )
    ...
    if total_original_features > 0 and total_merged_features > 0:
        reduction_percent = round(
            100.0
            * (total_original_features - total_merged_features)
    ...
    for gridcode, stats in merge_stats.items():
        ...
        if stats["original_features"] > 0:
            reduction = round(
                100.0
                * (stats["original_features"] - stats["merged_features"])
    ...
    performance_metrics = {
        ...
        "processing": {"num_chunks": len(chunks)},
    }
...
def test_output(output, *args) -> None:
    ...
    assert "input_path" in output, (
        "Output should contain input_path with path to GeoParquet file"
    )
...
if __name__ == "__main__":
    # For local testing
    test_data = {"input_path": "path/to/test.parquet", "metadata": {"source_crs": 4326}}
