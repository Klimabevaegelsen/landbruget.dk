...
def wetlands_format_geojson(data, *args, **kwargs):
    ...
    try:
        ...
        if "gridcode" in gdf.columns:
            ...
            if (
                "properties" in gdf.columns
                and gdf["properties"].iloc[0]
                and "gridcode" in gdf["properties"].iloc[0]
            ):
                gdf["gridcode"] = gdf["properties"].apply(
                    lambda x: int(x.get("gridcode", 0))
                )
        ...
        empty_count = (
            gdf["geometry"].apply(lambda g: g.is_empty if g is not None else True).sum()
        )
        if empty_count > 0:
            print(f"⚠️ Found {empty_count:,} empty geometries, dropping them")
            gdf = gdf[
                ~gdf["geometry"].apply(lambda g: g.is_empty if g is not None else True)
            ]

        # Fix invalid geometries
        invalid_count = (
            gdf["geometry"]
            .apply(lambda g: not shapely.is_valid(g) if g is not None else True)
            .sum()
        )
        if invalid_count > 0:
            ...
            gdf["geometry"] = gdf["geometry"].apply(
                ...
                if g is not None and not shapely.is_valid(g)
                else g
            )
        ...
        if final_invalid > 0:
            print(
                f"⚠️ Still have {final_invalid:,} invalid geometries after fixing, dropping them"
            )
            gdf = gdf[
                gdf["geometry"].apply(
                    lambda g: shapely.is_valid(g) if g is not None else False
                )
            ]
        ...
        return {"input_path": output_path, "metadata": metadata}
...
def test_output(data):
    ...
    assert metadata.get("output_format") == "geoparquet", (
        "Output format should be geoparquet"
    )
                    ...
                    "geometry": {
                        ...
                        "coordinates": [
                            [
                                ...
                                [10.61199079, 57.74291192],
                                [10.61199479, 57.74300171],
                                [10.6121627, 57.74299957],
                                [10.6121587, 57.74290978],
                            ]
                        ],
                    },
                    "properties": {"gridcode": 12, "toerv_pct": "6-12"},
                },
                {
                    "type": "Feature",
                    "geometry": {
                        ...
                        "coordinates": [
                            [
                                ...
                                [10.61913061, 57.74102406],
                                [10.61913463, 57.74111385],
                                [10.61879883, 57.74111814],
                                [10.61880284, 57.74120793],
                                [10.61930654, 57.74120149],
                                [10.61929851, 57.74102192],
                            ]
                        ],
                    },
                    "properties": {"gridcode": 60, "toerv_pct": ">12"},
                },
            ],
        },
        "metadata": {"source_crs": 4326, "total_features": 2, "num_batches": 1},
    }

    try:
        result = wetlands_format_geojson(test_case)
        print(
            f"Success - Converted {result['metadata'].get('feature_count', 'unknown')} features"
        )
