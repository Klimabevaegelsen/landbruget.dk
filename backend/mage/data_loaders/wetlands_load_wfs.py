...
def load_wetlands_data(**kwargs):
    ...
    try:
        ...
        print(
            f"🔄 Will process {total_features:,} features in {num_batches:,} batches of {batch_size:,}"
        )
        ...
        for batch_index in range(num_batches):
            ...
            metadata_items.append({"block_uuid": f"batch_{batch_index}"})
...
def test_output(*args):
    ...
    if isinstance(data, list) and len(data) == 2:
        ...
        assert len(data_items) == len(metadata_items), (
            "Data items and metadata items should have the same length"
        )

        # Check first data item has all required fields for wetlands_process_batch.py
        first_data_item = data_items[0]
        assert "batch_index" in first_data_item, (
            "Each data item should contain batch_index"
        )
        ...
        for field, expected_type in required_metadata_fields.items():
            assert field in first_data_item["metadata"], (
        ...
        assert "block_uuid" in first_metadata_item, (
            "Each metadata item should contain block_uuid"
        )
    else:
