    ...
    def discover_all_afgoerelser_pdfs(self, cvr: str, base_gcs_path: str) -> List[str]:
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    subdirectories = [line.strip() for line in result.stdout.strip().split('\n') if line.strip().endswith('/')]
                    ...
                    for subdir in subdirectories:
                        if cvr in subdir:  # This subdirectory might contain our CVR
                            try:
                                ...
                                potential_path = f"{subdir}{cvr}/pdfs/" if not subdir.endswith(f"{cvr}/") else f"{subdir}pdfs/"
                                ...
                                if subdir_result.returncode == 0:
                                    ...
                                    subdir_afgoerelser = [f for f in files if 'afgoerelser_' in f and f.endswith('.pdf')]
                                    # Avoid duplicates
    ...
    def extract_first_pages_pdf(self, pdf_path: str, num_pages: int = 2) -> bytes:
        ...
        try:
            ...
            with open(pdf_path, 'rb') as file:
                ...
                logger.debug(f"💰 Cost optimization: {pages_to_extract} pages ({new_size:,} bytes) vs full PDF ({original_size:,} bytes) - {savings_pct:.1f}% reduction")
...
- Sæt confidence score lavt hvis usikker"""
    ...
    def parallel_permit_checks(self, pdf_paths: List[str], doc_info_template: DocumentInfo, max_workers: int = 5) -> List[Dict]:
        """
...
Vær konservativ - det er bedre at have for mange grupper end at fejlagtigt samle forskellige faciliteter."""
            ...
            for group in grouping_result.get("groups", []):
                ...
                logger.info(f"📍 Address group: '{canonical}' ({len(group_addresses)} variants, confidence: {confidence:.2f})")
    ...
    def group_documents_by_facility(self, permit_results: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        for result in valid_documents:
            ...
            logger.debug(f"Grouped document {result.get('pdf_path', '').split('/')[-1]} under canonical facility: {canonical_address}")
        ...
        for canonical_address, documents in facility_groups.items():
            ...
            logger.info(f"🏭 Canonical facility '{canonical_address}': {len(documents)} documents (newest: {date_str}){variant_info}")
    ...
    def analyze_facility_documents(self, facility_address: str, documents: List[Dict], doc_info_template: DocumentInfo) -> Dict:
        """
...
```"""
    ...
    def analyze_dma_permits_for_company(self, cvr: str, company_name: str, base_gcs_path: str) -> Dict:
        ...
        doc_info_template = DocumentInfo(
            cvr=cvr,
            company_name=company_name,
            document_type="afgoerelser",
            document_path=""
        )
        
        # Step 1: Parallel permit checks (5-10x faster!) - extracts addresses early
        logger.info(f"⚡ Running parallel permit checks on {len(pdf_paths)} documents (extracting addresses for grouping)...")
        permit_results = self.parallel_permit_checks(pdf_paths, doc_info_template, max_workers=5)
        
        # Log permit check results
        valid_permits = sum(1 for r in permit_results if r.get("permit_check", {}).get("analysis", {}).get("is_environmental_permit", False))
...
def main():
    ...
    test_companies = [
        {
            "cvr": "39675706",
            "company_name": "Rosenfeldt v/Nicolai Oxholm Tillisch",
            "rank": 2
        },
        {
            "cvr": "43377531",
            "company_name": "Smedsgaard Agro ApS",
            "rank": 6
        }
    ]
        ...
        try:
            ...
            for facility_address, facility_result in result.get("facilities", {}).items():
                if facility_result.get("status") == "facility_analysis_complete":
                    ...
                    if nh3_emission:
                        logger.info(f"    🏭 {facility_address}: NH3 emission: {nh3_emission} kg/år (confidence: {confidence:.2f})")
            
            logger.info(f"✅ Company summary: {total_facilities} facilities, {total_permits}/{total_docs} environmental permits found")
