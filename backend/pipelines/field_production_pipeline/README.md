# Field Production Pipeline

This pipeline generates comprehensive agricultural field production estimates for each year. It combines agricultural field data with DST (Danish Statistics) yield data to provide accurate production estimates for individual fields based on crop type, area, and regional yield data.

## Features

- **Optimized for DuckDB Spatial v1.2.2**: Leverages the new SPATIAL_JOIN operator (single spatial predicate only, per PR #545)
- **Accurate Production Estimates**: Uses DST data to estimate production based on crop type, area, and regional yields
...
### Output Data
- **Field Production Data**: Comprehensive field data with production estimates saved to silver layer (`silver/field_production/{timestamp}/field_production_{year}.parquet`)

## Output Schema

The pipeline produces comprehensive field production data with the following columns:
