...
validate_success() {
    ...
    if gsutil ls gs://landbruget-data/silver/property_owners/ >/dev/null 2>&1; then
        ...
        today=$(date +%Y%m%d)
        recent_files=$(gsutil ls gs://landbruget-data/silver/property_owners/${today}*/data.parquet 2>/dev/null | wc -l)
        if [ "$recent_files" -gt 0 ]; then
            ...
            legacy_files=$(gsutil ls -l gs://landbruget-data/silver/property_owners/*.parquet 2>/dev/null | grep "$(date +%Y-%m-%d)" | wc -l)
