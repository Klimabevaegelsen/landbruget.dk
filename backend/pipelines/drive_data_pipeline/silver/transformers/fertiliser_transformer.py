    ...
    def transform(
        ...
        try:
            ...
            if harmonized_df is None or harmonized_df.empty:
                return TransformResult(
                    success=False,
                    error="No valid data after harmonization"
                )
    ...
    def _process_efterafgroeder(self, df: pd.DataFrame, filename: str) -> pd.DataFrame:
        ...
        if 'a18_indberetefterafgalternativ' in df.columns:
            ...
            harmonized_data['omregnet_areal_ha'] = pd.to_numeric(
                df['a20_omregnethamedea'].astype(str).str.replace(',', '.'), 
                errors='coerce'
            )
            ...
            if 'a21_omregnethamedea' in df.columns:
                # 2021 format
                harmonized_data['omregnet_areal_ha'] = pd.to_numeric(
                    df['a21_omregnethamedea'].astype(str).str.replace(',', '.'),
                    errors='coerce'
                )
            elif 'a23_omregnethamedea' in df.columns:
                # 2023 format
                harmonized_data['omregnet_areal_ha'] = pd.to_numeric(
                    df['a23_omregnethamedea'].astype(str).str.replace(',', '.'),
                    errors='coerce'
                )
            ...
            harmonized_data['omregnet_areal_ha'] = pd.to_numeric(
                df['a24_omregnethamedea'].astype(str).str.replace(',', '.'),
                errors='coerce'
            )
        else:
