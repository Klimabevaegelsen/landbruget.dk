...
class WorkPermitsTransformer(BaseTransformer):
    ...
    def __init__(self):
        ...
        self.country_names = [
            'Ukraine', 'Indien', 'Brasilien', 'Vietnam', 'Uganda', 
            'Bangladesh', 'Filippinerne', 'Tanzania', 'Thailand',
            'Kirgizstan', 'Usbekistan', 'Hviderusland', 'Sydafrika',
            'Kina', 'Ghana', 'Marokko', 'Polen', 'Rumænien', 'Tyrkiet',
            'Etiopien', 'Kenya', 'Pakistan', 'Albanien', 'Nordmakedonien'
        ]
    ...
    def can_handle(self, file_path: Path, file_metadata: Dict[str, Any]) -> bool:
        ...
        work_permit_indicators = [
            'landbrugsvisum', 'work_permit', 'arbejdstilladelse', 
                ...
                if country in line:
                    ...
                    if numbers:
                        ...
                        if permit_counts:
                            ...
                            for i, count in enumerate(permit_counts[:-1]):  # Skip the last (likely total)
                                if i < len(self.expected_years):
                                    ...
                                    logger.debug(f"Added record: {current_cvr} - {country} - {year}: {count} permits")
    ...
    def _clean_work_permits_data(self, records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        df_clean = df.groupby(['company_id', 'nationality', 'year']).agg({
            'first_permits_count': 'sum',
            'source_file': 'first',
            'extracted_at': 'first'
        }).reset_index()
