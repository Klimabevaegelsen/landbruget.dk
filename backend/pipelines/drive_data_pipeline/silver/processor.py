...
class SilverProcessor:
    ...
    def _list_bronze_files(
        ...
        for metadata_path in bronze_run_path.glob("**/*.metadata.json"):
            try:
                ...
                if supported_file_types and metadata.file_extension.lstrip('.') not in supported_file_types:
                ...
                if specific_subfolders and metadata.original_subfolder not in specific_subfolders:
                    logger.debug(f"Skipping file from unspecified subfolder: {metadata.original_subfolder}")
                    continue
    ...
    def _apply_schema_to_file(
        ...
        try:
            ...
            table_schema = self.schema_manager.get_schema_by_subfolder(
                metadata.original_subfolder
            )
            
            if not table_schema:
                logger.info(f"No schema found for {metadata.original_subfolder}, skipping schema application")
                return None
            ...
            schema_output_path = output_path.with_name(f"{output_path.stem}_schema{output_path.suffix}")
    ...
    def _handle_pii_in_file(self, output_path: Path, silver_run_path: Path) -> Path | None:
        ...
        try:
            ...
            if not validation_result.is_valid:
                ...
                # Save to new file
                pii_output_path = output_path.with_name(f"{output_path.stem}_pii_handled{output_path.suffix}")
