...
def reconstruct_stable_fires(con: duckdb.DuckDBPyConnection, drive_silver_dir: Path) -> bool:
    ...
    try:
        ...
        logger.info(f"✅ Reconstructed {con.execute('SELECT COUNT(*) FROM cleaned_fires').fetchone()[0]} stable fire events")
        return True
...
def get_date_columns(con: duckdb.DuckDBPyConnection, table_name: str) -> Dict[str, Optional[str]]:
    ...
    try:
        ...
        # Look for start date
        start_candidates = [col for col in column_names if any(word in col.lower() for word in ['start', 'from', 'begin'])]
        result['start'] = start_candidates[0] if start_candidates else None
        
        # Look for end date
        end_candidates = [col for col in column_names if any(word in col.lower() for word in ['end', 'slut', 'to', 'expir'])]
...
def create_animal_welfare_timeline_parts(con: duckdb.DuckDBPyConnection) -> List[str]:
    ...
    try:
        ...
        desc_col = next((col for col in column_names if any(word in col.lower() for word in ['indsats', 'description', 'område'])), 'NULL')
        species_col = next((col for col in column_names if any(word in col.lower() for word in ['dyre', 'species', 'art'])), "'Unknown'")
...
def create_vet_events_timeline_parts(con: duckdb.DuckDBPyConnection) -> List[str]:
    ...
    try:
        ...
        disease_col = next((col for col in column_names if any(word in col.lower() for word in ['sygdom', 'disease'])), 'NULL')
        status_col = next((col for col in column_names if 'status' in col.lower()), 'NULL')
        species_col = next((col for col in column_names if any(word in col.lower() for word in ['dyre', 'species', 'art'])), "'Unknown'")
        
        date_col = date_cols.get('date') or next((col for col in column_names if 'vet' in col.lower() and 'date' in col.lower()), None)
...
def create_veterinary_timeline(con: duckdb.DuckDBPyConnection, pipeline_run_date: str,
                             gcs_access: Optional[GCSDataAccess] = None) -> bool:
    """
...
def process_veterinary_timeline(export_timestamp: str, 
                              gold_dir: Optional[Path] = None,
                              gcs_access: Optional[GCSDataAccess] = None,
                              pipeline_run_date: Optional[str] = None) -> bool:
    """
