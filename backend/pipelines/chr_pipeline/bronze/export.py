...
def _serialize_data(data: Any) -> Optional[str]:
    ...
    try:
        ...
        logger.warning(
            f"TypeError during serialization: {type_error}. Attempting fallback serialization."
        )
        try:
...
def finalize_export(clear_buffer: bool = True):
    ...
    for buffer_key, format_data in _data_buffer.items():
        ...
        if json_data_list:
            ...
            if USE_GCS:
                try:
                    logger.info(
                        f"Writing {len(json_data_list)} records to GCS bucket '{GCS_BUCKET}': {filename}"
                    )
    ...
    logger.info(
        f"Export complete: {total_files} files written using {storage_mode} in bronze/chr/{EXPORT_TIMESTAMP}/"
    )
    if clear_buffer:
        _data_buffer.clear()
