...
def _create_base_request(
    username: str, session_id: str = "1", track_id: str = "load_besaetning"
) -> Dict[str, str]:
...
def load_herd_list(
    ...
    try:
        ...
        RequestParamsFactory = besaetning_client.get_type(
        ...
        if response is None:
            logger.warning(
                f"No response received for species {species_code}, usage {usage_code}, start {start_herd_number}."
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if herd_details_raw:
            logger.info(
                f"Successfully called load_herd_details for herd {TEST_HERD_NUMBER}. Raw data saved via export module."
            )
