...
def parse_args() -> Dict[str, Any]:
    ...
    parser.add_argument(
        "--test-species-codes", type=str, help='Comma-separated species codes (e.g., "12,13,14,15")'
    )
    parser.add_argument(
        "--limit-total-herds", type=int, help="Limit total number of herds processed"
    )
    parser.add_argument(
        "--limit-herds-per-species", type=int, help="Limit number of herds processed per species"
    )
    parser.add_argument(
        "--log-level",
...
def fetch_stamdata(
    client: Any, username: str, test_species_codes: Optional[List[int]] = None
) -> List[Dict]:
...
def get_required_steps(target_step: str) -> List[str]:
    ...
    required_bronze = [
        s for s in step_dependencies.get(target_step, []) if not s.startswith("silver_")
    ]
        ...
        context = {
            ...
            "clients": {
                "stamdata": create_stamdata_client(
                    STAMDATA_ENDPOINTS["stamdata"], username, password
                ),
