...
class FieldProductionEstimator:
        ...
        for table_name, measurement_units in dst_tables.items():
            try:
                ...
                if table_path.exists():
                    ...
                    logging.info(f"Loaded {table_name}: {len(df_filtered)} records")
                else:
                    logging.warning(f"{table_name} file not found at {table_path}")
    ...
    def _load_dst_zone_mapping(self) -> Optional[gpd.GeoDataFrame]:
        ...
        try:
            zone_mapping = gpd.read_parquet("data_cache/dst_zone_mapping/data.parquet")
            logging.info(f"Loaded DST zone mapping: {len(zone_mapping)} zones")
            return zone_mapping
        except FileNotFoundError:
            logging.warning("DST zone mapping not found, using national averages only")
            return None
...
def create_field_overview_optimized(
    year: int, estimator: FieldProductionEstimator, limit: Optional[int] = None
) -> pd.DataFrame:
    """Create comprehensive field overview with production estimates using optimized batch spatial processing."""
    ...
    logging.info(f"Creating optimized field overview for {year}...")
    ...
    logging.info(f"Optimized Overview Summary for {year}:")
...
def save_overview(overview_df: pd.DataFrame, year: int, output_dir: Path, gcs_storage: Optional[GCSStorage] = None):
    """Save the field overview to file and optionally upload to GCS."""
    # Create output directory
    output_dir.mkdir(parents=True, exist_ok=True)

    # Save locally first
    local_output_file = output_dir / f"field_overview_{year}.parquet"
    overview_df.to_parquet(local_output_file, index=False)

    logging.info(f"Saved field overview to: {local_output_file}")
    ...
    if gcs_storage:
        try:
            ...
            temp_file = f"/tmp/field_overview_{year}_{timestamp}.parquet"
            overview_df.to_parquet(temp_file, index=False)

            gcs_path = f"gs://landbrugsdata-raw-data/silver/field_overview/{timestamp}/field_overview_{year}.parquet"

            cmd = f"gsutil cp {temp_file} {gcs_path}"
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True)

            if result.returncode == 0:
                logging.info(f"✅ Uploaded field overview to GCS: {gcs_path}")
            else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate agricultural field overview with production estimates - Optimized for DuckDB Spatial v1.2.2"
    )
...
def main():
    """Main function to run the optimized field overview generation pipeline."""
    args = parse_args()
    ...
    for year in years:
        try:
            ...
            overview_df = create_field_overview_optimized(year, estimator, args.limit)
            save_overview(overview_df, year, output_dir, gcs_storage)
    ...
    logging.info(f"{'=' * 60}")
    logging.info("Optimized field overview generation complete!")
