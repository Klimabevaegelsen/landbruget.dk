...
class WetlandsBronze(BaseSource[WetlandsBronzeConfig], BronzeJobInterface):
    ...
    def create_dataframe(self, raw_data: list[str]):
        ...
        This method takes a list of strings and converts it into a DuckDB table.
        ...
        self.conn.execute("CREATE OR REPLACE TABLE temp_wetlands_data (payload VARCHAR)")

        # Use prepared statements to safely insert the data strings
        for data_str in raw_data:
            self.conn.execute("INSERT INTO temp_wetlands_data VALUES (?)", [data_str])
        ...
        self.conn.execute(
            ...
            SELECT 
                ...
                ? as created_at,
                ? as updated_at
            FROM temp_wetlands_data
        """,
            [self.config.name, current_timestamp, current_timestamp],
        )

        # Clean up the temporary table
        self.conn.execute("DROP TABLE temp_wetlands_data")
    ...
    async def run(self) -> Optional[list[str]]:
        ...
        async with AsyncTimer("Running Wetlands bronze job for"):
            ...
            # Create  for storage (backward compatibility)
