...
class SubsidiesGoldConfig(BaseJobConfig):
    ...
    type: str = "gold"
    description: str = "Company-level subsidies aggregations and analytics for agricultural subsidies"
...
class SubsidiesGold(BaseSource[SubsidiesGoldConfig], GoldJobInterface):
    ...
    async def run(self, silver_data: Optional[Dict[str, pd.DataFrame]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log.info(f"💰 Total subsidies: {results.get('total_subsidies_amount', 0):,.2f} DKK")
        ...
        try:
            ...
            for i, file_path in enumerate(files):
                try:
                    ...
                    data_conn.execute(f"CREATE TABLE {table_name} AS SELECT * FROM read_parquet('{full_gs_path}')")
    ...
    def _analyze_cvr_data(self, df: pd.DataFrame) -> None:
        ...
        company_columns = [col for col in df.columns if any(term in col.lower() for term in ['company', 'virksomhed', 'bedrift', 'organization', 'beneficiary'])]
        ...
        if 'cvr_number' in df.columns:
            ...
            self.log.info(f"   CVR Analysis:")
            self.log.info(f"     Total records: {total_records:,}")
            self.log.info(f"     Records with CVR: {non_null_cvr:,} ({non_null_cvr/total_records*100:.1f}%)")
        ...
        if 'year' in df.columns:
            ...
            self.log.info(f"   Year distribution:")
    ...
    def _extract_year_from_dates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.log.info(f"✅ Total records with year information: {years_extracted:,} out of {len(df):,}")
    ...
    def _calculate_subsidy_amounts(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            if any(term in col.lower() for term in amount_keywords):
                try:
                    ...
                    temp_col = temp_col.str.replace(',', '.').str.replace(' ', '').str.replace('kr', '')
        ...
        if df['amount_dkk'].sum() == 0:
            area_cols = [col for col in df.columns if 'area' in col.lower() or 'areal' in col.lower()]
            for area_col in area_cols:
                try:
                    ...
                    self.log.info(f"   Calculated from {area_col}: {area_amounts.sum():,.2f} DKK (rate: {subsidy_rate}/ha)")
                except Exception as e:
        ...
        for animal_col in animal_cols:
            try:
                ...
                self.log.info(f"   Calculated from {animal_col}: {animal_amounts.sum():,.2f} DKK (rate: {animal_rate}/animal)")
            except Exception as e:
        ...
        self.log.info(f"💰 Records with monetary amounts: {records_with_amounts:,} out of {len(df):,}")
    ...
    def _process_subsidies_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'cvr_number' in df.columns:
            ...
            self.log.info(f"   Initial: {initial_records:,} records, {initial_cvr_count:,} with CVR, {initial_companies:,} unique CVR numbers")
        else:
        ...
        if 'year' in df.columns:
            ...
            self.log.info(f"   Year data: {records_with_year:,} records with year, {records_without_year:,} without year (all kept)")
        ...
        self.log.info(f"   ✅ NO FILTERING APPLIED - All data preserved:")
    ...
    async def _generate_company_analytics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        summary_stats = self._generate_summary_stats(df, company_summary, yearly_breakdown, category_analysis)
    ...
    def _calculate_company_summary(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_with_cvr = df[df['cvr_number'].notna() & (df['cvr_number'] != '') & (df['cvr_number'] != 'nan')]

        if df_with_cvr.empty:
            self.log.warning("No records with valid CVR numbers found")
            return pd.DataFrame()

        self.log.info(f"   Analyzing {len(df_with_cvr):,} records with CVR numbers from {df_with_cvr['cvr_number'].nunique():,} companies")
        ...
        if amount_col:
            ...
            if total_amount_col in summary.columns:
                summary['company_size'] = pd.cut(
                    ...
                    bins=[0, self.config.small_company_threshold, self.config.medium_company_threshold,
                          self.config.large_company_threshold, float('inf')],
    ...
    def _calculate_yearly_breakdown(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        yearly.columns = ['_'.join(col).strip() if isinstance(col, tuple) else col for col in yearly.columns]
    ...
    def _calculate_category_analysis(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        category_analysis = df.groupby(['cvr_number', category_col]).agg({
        ...
        if total_col in category_analysis.columns:
            ...
            category_analysis = category_analysis.merge(
                company_totals.rename('company_total'),
                left_on='cvr_number',
                right_index=True
            )
        ...
        self.log.info(f"   Generated category analysis: {len(category_analysis):,} company-category records")
        return category_analysis

    def _generate_summary_stats(self, df: pd.DataFrame, company_summary: pd.DataFrame,
                              yearly_breakdown: pd.DataFrame, category_analysis: pd.DataFrame) -> Dict[str, Any]:
        ...
        if subsidy_categories:
            self.log.info(f"📋 Category breakdown: {len(subsidy_categories)} unique categories found")
        ...
        if top_subsidy_types:
            ...
            for category_type, categories in top_subsidy_types.items():
                ...
                for cat_name, cat_data in list(categories.items())[:5]:  # Show top 5
                    self.log.info(f"     • {cat_name}: {cat_data['total_amount']:,.2f} DKK ({cat_data['record_count']:,} records)")
    ...
    def _get_subsidy_categories(self, df: pd.DataFrame) -> List[str]:
        ...
        categories = []
        category_columns = ['subsidy_category', 'subsidy_measure', 'subsidy_type_code', 'ordning', 'silver_source_dataset']
        ...
        if categories:
            self.log.info(f"Found {len(categories)} subsidy categories. Examples: {categories[:10]}")
    ...
    def _analyze_subsidy_types_by_amount(self, df: pd.DataFrame) -> Dict[str, Dict]:
        ...
        analysis = {}
        category_columns = ['subsidy_measure', 'subsidy_type_code', 'ordning', 'silver_source_dataset']

        for col in category_columns:
            if col in df.columns:
                category_totals = df.groupby(col)['amount_dkk'].agg(['sum', 'count', 'mean']).round(2)
    ...
    async def _save_analytics_data(self, company_summary: pd.DataFrame,
                                 yearly_breakdown: pd.DataFrame,
                                 category_analysis: pd.DataFrame) -> None:
            ...
            for df, dataset_name in datasets:
                if not df.empty:
                    try:
                        ...
                        save_conn.execute(f"COPY {table_name} TO '{full_gcs_path}' (FORMAT PARQUET)")

                        self.log.info(f"   ✅ Saved {dataset_name}: {len(df):,} records to {output_path}")
