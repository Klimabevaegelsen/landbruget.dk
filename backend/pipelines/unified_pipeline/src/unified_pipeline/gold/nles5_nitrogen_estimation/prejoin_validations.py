...
class NLES5PrejoinValidator:
    ...
    def _validate_minimum_input_tables(self) -> None:
        ...
        if missing:
            raise ValueError(
                f"Missing required pre-join tables: {missing}. "
                f"Real silver data is required before joins."
            )

    @timed(name="§3.1 Validate fertilizer_history quality")
    def _validate_fertilizer_history_quality(self) -> None:
        self.log.info(
            "🧪 §3.1 Validating Gødningsregnskab (fertilizer_history) quality "
            "per N2023_62 Table 1..."
        )
        try:
            ...
            (
                ...
                negative_fertilizer,
                negative_total_n,
                missing_mineral_fertilizer,
                missing_cvr,
                invalid_landbrugsareal,
                negative_harmoniareal,
                missing_n_quota,
                excessive_n_quota,
                too_low_n_quota,
                excessive_n_consumption,
                livestock_no_type,
                consumption_mismatch_1,
                consumption_mismatch_2,
                utilization_inconsistency,
                low_utilization_rate,
                cover_crop_base_missing,
                cover_crop_base_exceeds,
                cover_crop_exceeds_base,
                livestock_production_mismatch,
            ) = stats

            # Log comprehensive statistics
            self.log.info(
                f"📊 fertilizer_history validation (N2023_62 Table 1): {total:,} total records"
            )
            self.log.info(
                f"   Basic issues: negative_fertilizer={negative_fertilizer:,}, "
                f"negative_total_n={negative_total_n:,}, missing_cvr={missing_cvr:,}"
            )
            self.log.info(
                f"   Area issues: invalid_landbrugsareal={invalid_landbrugsareal:,}, "
                f"negative_harmoniareal={negative_harmoniareal:,}"
            )
            self.log.info(
                f"   N-quota issues: missing={missing_n_quota:,}, "
                f"excessive(>400)={excessive_n_quota:,}, low(<10)={too_low_n_quota:,}, "
                f"consumption(>600)={excessive_n_consumption:,}"
            )
            self.log.info(
                f"   Livestock fertilizer issues: no_type={livestock_no_type:,}, "
                f"mismatch_1={consumption_mismatch_1:,}, mismatch_2={consumption_mismatch_2:,}"
            )
            self.log.info(
                f"   Utilization issues: inconsistent={utilization_inconsistency:,}, "
                f"low_rate(<30%)={low_utilization_rate:,}"
            )
            self.log.info(
                f"   Cover crop issues: base_missing={cover_crop_base_missing:,}, "
                f"base_exceeds={cover_crop_base_exceeds:,}, "
                f"exceeds_base={cover_crop_exceeds_base:,}"
            )
            ...
            if missing_cvr > 0:
                critical_issues.append(
                    f"{missing_cvr:,} records without CVR (required for linkage)"
                )
            if invalid_landbrugsareal > 0:
                critical_issues.append(
                    f"{invalid_landbrugsareal:,} records with invalid "
                    f"landbrugsareal (frasorteringskode=1)"
                )
            if negative_fertilizer > 0 or negative_total_n > 0:
                critical_issues.append(
                    f"{negative_fertilizer + negative_total_n:,} records with "
                    f"negative nitrogen quantities"
                )

            # N-quota critical thresholds (per Table 1)
            if missing_n_quota > 0:
                critical_issues.append(
                    f"{missing_n_quota:,} records with missing N-quota (frasorteringskode=2)"
                )
            if excessive_n_quota > 0:
                critical_issues.append(
                    f"{excessive_n_quota:,} records with N-quota >400 kg N/ha (frasorteringskode=3)"
                )
            if excessive_n_consumption > 0:
                critical_issues.append(
                    f"{excessive_n_consumption:,} records with N-consumption "
                    f">600 kg N/ha (frasorteringskode=3)"
                )
            if too_low_n_quota > 0:
                critical_issues.append(
                    f"{too_low_n_quota:,} records with N-quota <10 kg N/ha (frasorteringskode=4)"
                )
            if livestock_production_mismatch > 0:
                critical_issues.append(
                    f"{livestock_production_mismatch:,} records with livestock units "
                    f">1000 but norm production <50000 kg N (frasorteringskode=5)"
                )
            ...
            if missing_mineral_fertilizer > 0:
                warnings.append(
                    f"{missing_mineral_fertilizer:,} records with missing mineral "
                    f"fertilizer (can be calculated from inventory)"
                )
            if livestock_no_type > 0:
                warnings.append(
                    f"{livestock_no_type:,} records with livestock fertilizer but unknown type"
                )
            if consumption_mismatch_1 > 0 or consumption_mismatch_2 > 0:
                warnings.append(
                    f"{consumption_mismatch_1 + consumption_mismatch_2:,} records with "
                    f"livestock fertilizer consumption/utilization mismatch"
                )
            if utilization_inconsistency > 0:
                warnings.append(
                    f"{utilization_inconsistency:,} records with livestock "
                    f"fertilizer utilization inconsistency"
                )
            if low_utilization_rate > 0:
                warnings.append(
                    f"{low_utilization_rate:,} records with livestock "
                    f"fertilizer utilization rate <30%"
                )
            if (
                cover_crop_base_missing > 0
                or cover_crop_base_exceeds > 0
                or cover_crop_exceeds_base > 0
            ):
                total_cc = (
                    cover_crop_base_missing + cover_crop_base_exceeds + cover_crop_exceeds_base
                )
                warnings.append(f"{total_cc:,} records with cover crop area inconsistencies")
    ...
    def _validate_fieldplan_vs_accounts_linkage(self) -> None:
        self.log.info(
            "🔗 §3.2 Validating linkage between field_plan and "
            "fertilizer_history by CVR/adresse-id..."
        )
        ...
        try:
            ...
            if total == 0 or good == 0:
                raise ValueError(
                    "§3.2 linkage validation failed: no CVR matches within 5%/2ha threshold"
                )
            if good / total < 0.90:
                # Expect high-quality linkage as per Table 3 (≈99% in report)
                raise ValueError(
                    f"§3.2 linkage validation failed: only "
                    f"{good / total * 100:.1f}% within 5%/2ha (target ≥ 90%)"
                )
    ...
    def _validate_agricultural_fields_integrity(self) -> None:
        ...
        try:
            ...
            self.log.info(
                f"📊 agricultural_fields_spatial: {total:,} rows | "
                f"null geom={null_geom:,} | invalid geom={invalid_geom:,} | "
                f"invalid area={invalid_area:,}"
            )
    ...
    def _log_table1_coverage(self) -> None:
        ...
        self.log.info(
            "   ✅ Row 18: Livestock units >1000, norm production <50000 → "
            "Validation (frasorteringskode=5)"
        )
