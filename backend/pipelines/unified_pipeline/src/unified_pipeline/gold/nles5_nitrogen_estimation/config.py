...
class NLES5NitrogenEstimationGoldConfig(BaseJobConfig):
    ...
    dmi_evaporation_dataset: str = (
        "dmi_pot_evaporation_makkink_dmi_pot_evaporation_makkink"  # DMI potential evaporation
    )
    ...
    tessellation_batch_size: int = int(
        os.getenv("TESSELLATION_BATCH_SIZE", "10000")
    )  # Reduced from 25000 to prevent disk overflow
    spatial_join_batch_size: int = int(
        os.getenv("SPATIAL_JOIN_BATCH_SIZE", "1000")
    )  # Ultra-small batches to prevent memory exhaustion
    nles5_calculation_batch_size: int = int(
        os.getenv("NLES5_CALCULATION_BATCH_SIZE", "10000")
    )  # Reduced from 20000 for memory optimization
    ...
    # OPTIMIZED YEAR SELECTION: Only loads years actually needed for NLES5
    # calculations. Specify target calculation years - pipeline automatically
    # loads required supporting years (current + 2 previous).
    # Example: target_years = [2021, 2022, 2023] → loads
    ...
    enable_pipeline_batching: bool = bool(
        os.getenv("ENABLE_PIPELINE_BATCHING", "true").lower() == "true"
    )
    target_year_batch_size: int = int(
        os.getenv("TARGET_YEAR_BATCH_SIZE", "1")
    )  # Years per pipeline batch (1 for modularity)
    ...
    coefficient_uncertainties: Dict[str, float] = {
        ...
        "Bcs": 0.007000,
        # βCA: Mineral N application in autumn (SE from DCA Rapport 163 Table 3.2)
        "Bca": 0.034257,
        # βudb: Mineral N deposited by grazing animals
        # (SE from DCA Rapport 163 Table 3.2)
        "Budb": 0.011056,
        # βm1: Effect of mineral and organic N in previous two years
        # (SE from DCA Rapport 163 Table 3.2)
        "Bm1": 0.006121,
        # βf0: Biological N fixation in current year
        # (SE from DCA Rapport 163 Table 3.2)
        "Bf0": 0.005530,
        # βf1: Biological N fixation in previous two years
        # (SE from DCA Rapport 163 Table 3.2)
        "Bf1": 0.006121,
        # βg0: Organic N in animal manure in current year
        # (SE from DCA Rapport 163 Table 3.2)
        "Bg0": 0.008799,
    }
