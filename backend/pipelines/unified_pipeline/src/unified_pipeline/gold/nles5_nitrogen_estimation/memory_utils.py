...
This module contains all memory management and cleanup utilities for
NLES5 nitrogen washout estimation. It includes:
...
All methods maintain the exact same functionality and cleanup strategies
...
class NLES5MemoryUtils:
    ...
    def _monitor_memory_usage(self, operation_name: str) -> None:
        ...
        try:
            ...
            if current_memory > self.config.max_memory_usage_gb * 0.8:
                self.log.warning(
                    f"⚠️  High memory usage after {operation_name}: "
                    f"{current_memory:.2f} GB (limit: {self.config.max_memory_usage_gb} GB)"
                )

            # Perform cleanup if memory usage is too high
            if current_memory > self.config.max_memory_usage_gb * 0.9:
                self.log.warning(
                    f"🧹 Triggering aggressive cleanup due to high memory usage: "
    ...
    def _aggressive_memory_cleanup(self) -> None:
        ...
        try:
            ...
            if memory_freed > 0.1:  # Only log if significant memory was freed
                self.log.info(
                    f"🧹 Aggressive cleanup completed: freed {memory_freed:.2f} GB "
        ...
        try:
            ...
            try:
                ...
                for table_name in table_names:
                    if any(
                        pattern.replace("*", "") in table_name
                        for pattern in target_year_tables
                        if "*" in pattern
                    ):
                        try:
            ...
            for table_name in table_names:
                ...
                for pattern in pattern_table_patterns:
                    if pattern in table_name:
                        ...
                        if (
                            "enhanced_gkea_fvm_matches" in table_name
                            and "_" in table_name.split("enhanced_gkea_fvm_matches")[-1]
                        ) or (
                            "gkea_fvm_enhanced_mappings" in table_name
                            and "_" in table_name.split("gkea_fvm_enhanced_mappings")[-1]
                        ):
    ...
    def _aggressive_pipeline_cleanup(self) -> None:
        ...
        try:
            ...
            tables_to_preserve = [
                ...
                # FIXED: Preserve final batched results table during cleanup
    ...
    def _cleanup_temp_files(self) -> None:
        ...
        try:
            ...
            files_removed = 0
            space_freed = 0
            duckdb_temp_files_skipped = 0

            # 🔍 DIAGNOSTIC: Log which files are being cleaned
            ...
            for root, dirs, files in os.walk(temp_dir):
                for file in files:
                    ...
                    try:
                        ...
                        # 🔍 DIAGNOSTIC: Log file being removed
                        self.log.debug(
                            f"🔍 CLEANUP: Removing {file} ({file_size / (1024 * 1024):.2f} MB)"
                        )
                        os.remove(file_path)
                        files_removed += 1
            ...
            if files_removed > 0:
                ...
                self.log.info(
                    f"🧹 Cleaned {files_removed} temp files, freed {space_freed_mb:.1f} MB"
                )
                if duckdb_temp_files_skipped > 0:
                    self.log.info(
                        f"🔍 Skipped {duckdb_temp_files_skipped} active DuckDB temp files"
                    )
    ...
    def _check_memory_threshold(self, operation_name: str) -> bool:
        ...
        try:
            ...
            if current_memory > threshold:
                self.log.warning(
                    f"⚠️  Memory threshold exceeded before {operation_name}: "
                    f"{current_memory:.2f} GB > {threshold:.2f} GB"
                )
                return False
    ...
    def _log_memory_summary(self) -> None:
        ...
        try:
            ...
            self.log.info(
                f"   Process memory: {current_memory:.2f} GB "
                f"({memory_percentage:.1f}% of {memory_limit} GB limit)"
            )
            self.log.info(
                f"   System memory: {system_memory_gb:.1f} GB total, {system_usage_pct:.1f}% used"
            )
    ...
    def _ensure_memory_available(self, required_gb: float, operation_name: str) -> None:
        ...
        try:
            ...
            if available_memory < required_gb:
                self.log.warning(
                    f"⚠️  Insufficient memory for {operation_name}: "
                    f"need {required_gb:.1f} GB, have {available_memory:.1f} GB available"
                )
