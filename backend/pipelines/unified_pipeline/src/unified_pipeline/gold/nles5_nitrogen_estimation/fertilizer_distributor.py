...
class NLES5FertilizerDistributor:
    ...
    def distribute_fertilizer_for_farm(
        ...
        for field in field_data:
            priority = self.get_crop_priority(field.get("crop_name", ""), field.get("m_code", ""))
        ...
        organic_quota_ratio = (
            farm_budget.total_organic_n_kg / total_n_quota if total_n_quota > 0 else 0
        )

        if organic_quota_ratio > 0.5:
            # Proportional distribution (organic > 50% of quota)
            allocations = self._distribute_proportional(
                farm_budget, fields_with_priority, total_n_quota
            )
            method = "proportional"
        else:
        ...
        self.log.debug(
            f"Completed fertilizer distribution for CVR {farm_budget.cvr_number} "
    ...
    def _distribute_proportional(
        ...
        for field in fields:
            ...
            allocation = FieldFertilizerAllocation(
                ...
                mineral_n_rate_kg_ha=mineral_n_allocated / field["area_ha"]
                if field["area_ha"] > 0
                else 0,
                organic_quota_fraction=organic_n_allocated / field["n_quota_kg"]
    ...
    def _distribute_priority_based(
        ...
        for priority in sorted(priority_groups.keys()):
            ...
            if total_group_capacity > 0:
                ...
                for field in group_fields:
                    ...
                    field_fraction = (
                        field_capacity / total_group_capacity if total_group_capacity > 0 else 0
                    )
        ...
        for field in fields:
            allocation = FieldFertilizerAllocation(
                ...
                mineral_n_rate_kg_ha=field["mineral_n_allocated"] / field["area_ha"]
                if field["area_ha"] > 0
                else 0,
                organic_quota_fraction=field["organic_n_allocated"] / field["n_quota_kg"]
                if field["n_quota_kg"] > 0
                else 0,
    ...
    def apply_fertilizer_distribution_to_pipeline(self, fields_table: str) -> str:
        ...
        self.log.info(
            f"Creating distributed fertilizer table with "
            f"{len(all_allocations)} field allocations..."
        )
        ...
        self.log.info(
            f"✅ NLES5 fertilizer distribution completed: {distributed_count:,} fields processed, "
