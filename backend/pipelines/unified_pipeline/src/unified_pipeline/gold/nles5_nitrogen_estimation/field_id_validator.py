...
class FieldIDValidator:
    ...
    def validate_field_id_format(self, field_id: str) -> Dict[str, Any]:
        ...
        if field_id_str.isdigit():
            ...
            if len(field_id_str) > 10:
                validation_result['recommendations'].append('Very long numeric field ID - verify correctness')
            return validation_result
        ...
        if '-' in field_id_str:
            ...
            if len(parts) == 2:
                ...
                if left_part.isdigit() and right_part.isdigit():
                    ...
                    validation_result['is_valid_format'] = True
                    validation_result['recommendations'].append('Field ID uses FVM number-dash-number format (valid)')
                    return validation_result
                else:
                    validation_result['format_type'] = 'invalid_dash_format'
                    validation_result['issues'].append('Dash-separated field ID contains non-numeric parts')
            else:
                validation_result['format_type'] = 'invalid_dash_format'
            ...
            if len(parts) == 2:
                ...
                if cvr_part.isdigit() and len(cvr_part) == 8:
                    if marknummer_part.isdigit():
                        validation_result['is_valid_format'] = True
                        validation_result['recommendations'].append('Field ID uses CVR_Marknummer format (acceptable)')
                    else:
        ...
        # Check for alphanumeric format
            ...
            validation_result['is_valid_format'] = True
            validation_result['recommendations'].append('Field ID uses alphanumeric format - verify this is expected')
        
        # Unknown format
        else:
            validation_result['format_type'] = 'unknown'
            validation_result['issues'].append(f'Field ID "{field_id_str}" does not match any expected format (numeric, number-dash-number, UUID, CVR_Marknummer)')
            validation_result['recommendations'].append('Check if field ID contains invalid characters or unexpected format')
        ...
        try:
            ...
            if hasattr(parsed_uuid, 'version') and parsed_uuid.version:
                ...
                if parsed_uuid.version == 5:
                    validation_result['recommendations'].append('Field UUID uses UUID5 format (geometry-based, preferred for FVM)')
                elif parsed_uuid.version == 4:
                    validation_result['recommendations'].append('Field UUID uses UUID4 format (random)')
                else:
                    validation_result['recommendations'].append(f'Field UUID uses UUID{parsed_uuid.version} format')
            ...
            validation_result['issues'].append(f'Field UUID "{field_uuid_str}" is not a valid UUID: {e}')
            validation_result['recommendations'].append('Ensure field UUID follows proper UUID format (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx)')
    ...
    def collect_field_ids_before_processing(self) -> Set[str]:
        ...
        try:
                    ...
                    try:
                        ...
                        if count > 0:
                            ...
                            if 'field_id' in available_columns:
                                ...
                                self.log.info(f"Collected {len(table_field_ids):,} unique field IDs from {table_name}")
                            ...
                            if 'field_uuid' in available_columns:
                                ...
                                table_field_uuids = {str(row[0]) for row in field_uuid_results if row[0]}
                                field_uuids.update(table_field_uuids)
                                self.log.info(f"Collected {len(table_field_uuids):,} unique field UUIDs from {table_name}")
            ...
            self.log.info(f"✅ Collected {len(field_ids):,} field IDs and {len(field_uuids):,} field UUIDs before processing")
            ...
            if field_uuids:
                ...
                for sample_uuid in sample_uuids:
                    ...
                    if validation['is_valid_format']:
                        version_info = f"(UUID{validation.get('uuid_version', '?')})" if 'uuid_version' in validation else ""
                        self.log.info(f"   ✅ {sample_uuid}: {validation['format_type']} format {version_info}")
                    else:
                        self.log.warning(f"   ⚠️ {sample_uuid}: {validation['format_type']} format - {validation['issues']}")
            ...
            # Also validate field IDs if no UUIDs available
                ...
                for sample_id in sample_ids:
                    ...
                    if validation['is_valid_format']:
                        ...
                        self.log.warning(f"   ⚠️ {sample_id}: {validation['format_type']} format - {validation['issues']}")
    ...
    def collect_field_ids_after_processing(self) -> Set[str]:
        ...
        try:
            ...
            self.log.info(f"✅ Collected {len(field_ids):,} field IDs and {len(field_uuids):,} field UUIDs after processing")
    ...
    def validate_field_id_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.field_ids_before_processing and self.field_ids_after_processing:
                validation_result['field_ids_lost'] = self.field_ids_before_processing - self.field_ids_after_processing
                validation_result['field_ids_gained'] = self.field_ids_after_processing - self.field_ids_before_processing
                validation_result['field_ids_preserved'] = self.field_ids_before_processing & self.field_ids_after_processing
                
                if self.field_ids_before_processing:
                    validation_result['field_id_consistency_score'] = len(validation_result['field_ids_preserved']) / len(self.field_ids_before_processing)
            
            # Validate field UUIDs (primary validation)
            if self.field_uuids_before_processing and self.field_uuids_after_processing:
                validation_result['field_uuids_lost'] = self.field_uuids_before_processing - self.field_uuids_after_processing
                validation_result['field_uuids_gained'] = self.field_uuids_after_processing - self.field_uuids_before_processing
                validation_result['field_uuids_preserved'] = self.field_uuids_before_processing & self.field_uuids_after_processing
                
                if self.field_uuids_before_processing:
                    validation_result['field_uuid_consistency_score'] = len(validation_result['field_uuids_preserved']) / len(self.field_uuids_before_processing)
            
            # Calculate overall consistency score (prioritize UUIDs if available)
            if validation_result['field_uuid_consistency_score'] > 0:
                validation_result['consistency_score'] = validation_result['field_uuid_consistency_score']
                primary_validation = 'field_uuid'
            elif validation_result['field_id_consistency_score'] > 0:
                validation_result['consistency_score'] = validation_result['field_id_consistency_score']
                primary_validation = 'field_id'
            else:
                validation_result['issues'].append('No field identifiers found for consistency validation')
                validation_result['recommendations'].append('Ensure field identifier collection runs before and after processing')
                return validation_result
            ...
            if validation_result['total_field_ids_before'] > 0:
                self.log.info(f"   Field IDs before processing: {validation_result['total_field_ids_before']:,}")
                self.log.info(f"   Field IDs after processing: {validation_result['total_field_ids_after']:,}")
                self.log.info(f"   Field IDs preserved: {len(validation_result['field_ids_preserved']):,}")
                self.log.info(f"   Field IDs lost: {len(validation_result['field_ids_lost']):,}")
                self.log.info(f"   Field IDs gained: {len(validation_result['field_ids_gained']):,}")
                self.log.info(f"   Field ID consistency score: {validation_result['field_id_consistency_score']:.2%}")
            
            # Log field UUID results if available
            if validation_result['total_field_uuids_before'] > 0:
                self.log.info(f"   Field UUIDs before processing: {validation_result['total_field_uuids_before']:,}")
                self.log.info(f"   Field UUIDs after processing: {validation_result['total_field_uuids_after']:,}")
                self.log.info(f"   Field UUIDs preserved: {len(validation_result['field_uuids_preserved']):,}")
                self.log.info(f"   Field UUIDs lost: {len(validation_result['field_uuids_lost']):,}")
                self.log.info(f"   Field UUIDs gained: {len(validation_result['field_uuids_gained']):,}")
                self.log.info(f"   Field UUID consistency score: {validation_result['field_uuid_consistency_score']:.2%}")
            
            self.log.info(f"   Overall consistency score ({primary_validation}): {validation_result['consistency_score']:.2%}")
            
            # Check for UUID-specific issues (primary concern)
            if validation_result['field_uuids_lost']:
                validation_result['issues'].append(f"{len(validation_result['field_uuids_lost'])} field UUIDs were lost during processing")
                validation_result['recommendations'].append('Investigate why field UUIDs were lost during processing')
            ...
            if validation_result['field_uuids_gained']:
                validation_result['issues'].append(f"{len(validation_result['field_uuids_gained'])} field UUIDs were gained during processing")
                validation_result['recommendations'].append('Investigate why new field UUIDs were created during processing')
            ...
            if validation_result['field_ids_lost']:
                validation_result['issues'].append(f"{len(validation_result['field_ids_lost'])} field IDs were lost during processing")
            ...
            if validation_result['field_ids_gained']:
                validation_result['issues'].append(f"{len(validation_result['field_ids_gained'])} field IDs were gained during processing")
            ...
            if validation_result['consistency_score'] < 0.95:
                validation_result['issues'].append(f'Low consistency score: {validation_result["consistency_score"]:.2%}')
                validation_result['recommendations'].append('Investigate field identifier consistency issues')
            elif validation_result['consistency_score'] < 0.99:
                validation_result['recommendations'].append('Minor field identifier consistency issues detected')
            else:
                self.log.info(f"✅ Field identifier consistency is excellent ({primary_validation})")
    ...
    def validate_field_id_formats_in_dataset(self, table_name: str, sample_size: int = 100) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if 'field_id' in available_columns:
                try:
                    ...
                    for field_id in sample_field_ids:
                        ...
                        validation_result['field_id_format_distribution'][format_type] = validation_result['field_id_format_distribution'].get(format_type, 0) + 1
            ...
            if 'field_uuid' in available_columns:
                try:
                    ...
                    for field_uuid in sample_field_uuids:
                        ...
                        format_type = validation['format_type']
                        validation_result['field_uuid_format_distribution'][format_type] = validation_result['field_uuid_format_distribution'].get(format_type, 0) + 1
            ...
            if validation_result['records_with_field_id'] > 0:
                total_field_id_sampled = validation_result['field_id_valid_formats'] + validation_result['field_id_invalid_formats']
                if total_field_id_sampled > 0:
                    valid_percentage = validation_result['field_id_valid_formats'] / total_field_id_sampled
                    invalid_percentage = validation_result['field_id_invalid_formats'] / total_field_id_sampled
                    
                    self.log.info(f"   Field ID Results:")
                    self.log.info(f"     Records with field_id: {validation_result['records_with_field_id']:,}")
                    self.log.info(f"     Sampled field IDs: {total_field_id_sampled}")
                    self.log.info(f"     Valid formats: {validation_result['field_id_valid_formats']} ({valid_percentage:.1%})")
                    self.log.info(f"     Invalid formats: {validation_result['field_id_invalid_formats']} ({invalid_percentage:.1%})")
                    self.log.info(f"     Format distribution: {validation_result['field_id_format_distribution']}")
                    
                    if invalid_percentage > 0.1:
                        validation_result['issues'].append(f"High percentage of invalid field ID formats: {invalid_percentage:.1%}")
                        validation_result['recommendations'].append('Investigate and fix invalid field ID formats')
            
            # Log field UUID results (primary validation)
            if validation_result['records_with_field_uuid'] > 0:
                total_field_uuid_sampled = validation_result['field_uuid_valid_formats'] + validation_result['field_uuid_invalid_formats']
                if total_field_uuid_sampled > 0:
                    valid_percentage = validation_result['field_uuid_valid_formats'] / total_field_uuid_sampled
                    invalid_percentage = validation_result['field_uuid_invalid_formats'] / total_field_uuid_sampled
                    
                    self.log.info(f"   Field UUID Results:")
                    self.log.info(f"     Records with field_uuid: {validation_result['records_with_field_uuid']:,}")
                    self.log.info(f"     Sampled field UUIDs: {total_field_uuid_sampled}")
                    self.log.info(f"     Valid formats: {validation_result['field_uuid_valid_formats']} ({valid_percentage:.1%})")
                    self.log.info(f"     Invalid formats: {validation_result['field_uuid_invalid_formats']} ({invalid_percentage:.1%})")
                    self.log.info(f"     Format distribution: {validation_result['field_uuid_format_distribution']}")
                    
                    if invalid_percentage > 0.1:
                        validation_result['issues'].append(f"High percentage of invalid field UUID formats: {invalid_percentage:.1%}")
                        validation_result['recommendations'].append('Investigate and fix invalid field UUID formats')
                    elif invalid_percentage == 0:
                        self.log.info("✅ All field UUID formats are valid")
            
            if not validation_result['records_with_field_id'] and not validation_result['records_with_field_uuid']:
                validation_result['issues'].append('No field identifiers found in table')
                validation_result['recommendations'].append('Ensure table contains field_id or field_uuid columns')
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for table_name in key_tables:
                try:
                    format_validation = self.validate_field_id_formats_in_dataset(table_name, sample_size=50)
            ...
            consistency_score = comprehensive_results['consistency_validation'].get('consistency_score', 0.0)
            
            # Calculate format score from validations (prioritize UUID formats)
            format_scores = []
            uuid_presence_scores = []
            
            for table_name, format_validation in comprehensive_results['format_validations'].items():
                if 'error' not in format_validation:
                    # Prioritize field_uuid validation over field_id validation
                    uuid_total = format_validation.get('field_uuid_valid_formats', 0) + format_validation.get('field_uuid_invalid_formats', 0)
                    if uuid_total > 0:
                        uuid_score = format_validation.get('field_uuid_valid_formats', 0) / uuid_total
                        format_scores.append(uuid_score)
                    else:
                        # Fall back to field_id validation if no UUIDs
                        id_total = format_validation.get('field_id_valid_formats', 0) + format_validation.get('field_id_invalid_formats', 0)
                        if id_total > 0:
            ...
            self.log.info(f"   Overall validation score: {comprehensive_results['overall_score']:.2%}")
            self.log.info(f"   Validation status: {'✅ PASSED' if comprehensive_results['passed'] else '❌ FAILED'}")
    ...
    def validate_field_uuid_presence(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if validation_result['total_records'] > 0:
                    validation_result['field_uuid_coverage'] = validation_result['records_with_field_uuid'] / validation_result['total_records']
            ...
            if validation_result['field_uuid_coverage'] < 0.95:
                validation_result['issues'].append(f"Low field_uuid coverage: {validation_result['field_uuid_coverage']:.1%}")
                validation_result['recommendations'].append('Investigate why some records lack field_uuid values')
            elif validation_result['field_uuid_coverage'] < 1.0:
                validation_result['recommendations'].append('Minor field_uuid coverage gaps detected')
            
            if validation_result['uuid_format_issues'] > 0:
                validation_result['issues'].append(f"{validation_result['uuid_format_issues']} field UUIDs have format issues")
            ...
            self.log.info(f"   Has field_uuid column: {'✅' if validation_result['has_field_uuid_column'] else '❌'}")
            self.log.info(f"   Total records: {validation_result['total_records']:,}")
            self.log.info(f"   Records with field_uuid: {validation_result['records_with_field_uuid']:,}")
