...
This module handles the high-level pipeline orchestration logic for the
NLES5 nitrogen estimation system. It manages async execution, batching
strategies, target year processing, and overall pipeline coordination.
...
class NLES5PipelineOrchestrator:
    ...
    async def run(self, silver_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        try:
            ...
            self.log.info(
                ...
                f"{self.config.max_memory_usage_gb}GB memory limit"
            )
            self.log.info("🌍 Processing entire Denmark")

            if self.config.max_years_to_process:
                self.log.info(
                    f"📅 Years limit: {self.config.max_years_to_process} years "
                    f"(for disk space management)"
                )
            else:
            ...
            if self.config.enable_pipeline_batching:
                self.log.info(
                    f"🔄 Pipeline batching enabled: "
                    f"{self.config.target_year_batch_size} years per batch"
                )
    ...
    async def _run_pipeline_batched(self, silver_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.log.info(
            f"   💾 Maximum memory footprint: "
            f"{self.config.target_year_batch_size + 2} years (batch + 2 previous)"
        )

        # NOTE: Comprehensive data validation will run after silver data
        # loading in each batch

        # Initialize final results table with proper schema
        # FIXED: Ensure area_ha supports large values
        # Note: field_uuid is now preserved throughout pipeline from source
        self.conn.execute("""
        ...
        for batch_num, batch_years in enumerate(target_year_batches, 1):
            ...
            self.log.info("")
            self.log.info(
                f"🔄 ========== PIPELINE BATCH {batch_num}/{len(target_year_batches)} =========="
            )
        ...
        # Verify UUIDs in final batched results (no join needed - preserved throughout pipeline)
        self.log.info("✅ Verifying field UUIDs in final batched results...")
        self.processor._add_field_uuids_to_gold_table("nles5_estimates_final_batched")

        # Re-collect field IDs/UUIDs after UUID verification
        self.log.info("🔍 Re-collecting field IDs after UUID verification for batched results...")
    ...
    async def _run_pipeline_single(self, silver_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        if len(loaded_tables) < 2:  # At least some datasets
            self.log.error(
                "Insufficient data loaded - need at least climate data and other datasets"
            )
            return
    ...
    async def _run_pipeline_for_batch(
        ...
        try:
            ...
            phase_start = time.time()
            loaded_tables = self.processor._load_required_silver_datasets_for_batch(
                silver_data, batch_years
            )
            ...
            agricultural_fields_table = self.processor._load_agricultural_fields_data_for_batch(
            ...
            if agricultural_fields_table:
                ...
                self.log.info(
                    f"🔍 Validating field IDs before batch processing for years {batch_years}..."
                )
                self.processor._validate_field_ids_before_processing()
            else:
            ...
            self.processor._create_spatial_tables()
            self.processor._create_nles5_parameter_tables()
            # CRITICAL: Create fertilizer_history table
            self.processor._prepare_nitrogen_inputs_tables()
            phase_time = time.time() - phase_start
            ...
            if not validation_results["passed"]:
                error_msg = (
                    f"Batch {batch_years} validation failed - "
                    f"required real data is missing or invalid"
                )
                self.log.error(f"❌ {error_msg}")
                for error in validation_results["errors"]:
                    self.log.error(f"   - {error}")
                self.log.error(
                    "🚫 NO FALLBACK DATA WILL BE CREATED - batch requires complete real data"
                )
            ...
            phase_start = time.time()
            estimates_table = self._process_nles5_target_year_by_target_year_for_batch(
            ...
            if estimates_table:
                result_count = self.conn.execute(
                    f"SELECT COUNT(*) FROM {estimates_table}"
                ).fetchone()[0]
                if result_count > 0:
                    ...
                    """).fetchone()
                    self.log.info(
                        f"🔍 DEBUG area_ha stats for {estimates_table}: "
                        f"min={area_stats[0]:.4f}, max={area_stats[1]:.4f}, "
                        f"avg={area_stats[2]:.4f}, count={area_stats[3]:,}"
                    )
                    ...
                    if source_stats[2] == 0:
                        self.log.info(
                            f"✅ SOURCE TABLE {estimates_table}: {source_stats[0]:,} rows, "
                            f"{source_stats[1]:,} unique field_uuids, 0 duplicates (deduplicated)"
                        )
                    else:
                        self.log.warning(
                            f"⚠️ SOURCE TABLE {estimates_table}: "
                            f"{source_stats[0]:,} rows, "
                            f"{source_stats[1]:,} unique field_uuids, "
                            f"{source_stats[2]:,} duplicates"
                        )
                    ...
                    """).fetchone()
                    self.log.info(
                        f"🔍 BEFORE INSERT: nles5_estimates_final_batched has "
                        f"{before_stats[0]:,} rows, {before_stats[1]:,} unique field_uuids"
                    )
                    ...
                    self.conn.execute(f"""
                        ...
                        SELECT 
                            ...
                            cvr_number,
                            year,
                            -- Explicit cast to prevent DECIMAL(2,1) error
                            CAST(area_ha AS DECIMAL(10,4)) as area_ha,
                            crop_type,
                            soil_code,
                            soil_description,
                            clay_content,
                    ...
                    if after_stats[2] == 0:
                        self.log.info(
                            f"✅ AFTER INSERT: nles5_estimates_final_batched has "
                            f"{after_stats[0]:,} rows, {after_stats[1]:,} unique "
                            f"field_uuids, 0 duplicates"
                        )
                    else:
                        self.log.warning(
                            f"⚠️ AFTER INSERT: nles5_estimates_final_batched has "
                            f"{after_stats[0]:,} rows, {after_stats[1]:,} unique "
                            f"field_uuids, {after_stats[2]:,} TOTAL DUPLICATES"
                        )

                    batch_time = time.time() - batch_start
                    self.log.info(
                        f"✅ Batch {batch_years} results appended: "
                        f"{result_count:,} estimates in {batch_time:.1f}s"
                    )

                    # Validate field IDs after batch processing
                    self.log.info(
                        f"🔍 Validating field IDs after batch processing for years {batch_years}..."
                    )
                    self.processor._validate_field_ids_after_processing()
    ...
    def _process_single_target_year(
        ...
        only its 3-year data window.

        This method loads only the minimal data needed for one target year
        and processes it completely before cleanup. This ensures memory usage
        never exceeds the footprint of 3 years of data.
        ...
        try:
            self.log.info(
                f"   🔄 Loading agricultural fields for 3-year window: {sorted(required_years)}"
            )

            # Step 1: Load ONLY the agricultural fields data for required years
            self.processor._load_agricultural_fields_for_years(
                required_years, f"fields_target_{target_year}"
            )
            ...
            fields_climate_table = self.processor._spatial_join_year_climate(
            ...
            try:
                ...
                climate_matched = self.conn.execute(
                    f"SELECT COUNT(*) FROM {fields_climate_table} "
                    f"WHERE total_percolation IS NOT NULL"
                ).fetchone()[0]
                ...
                self.log.info(
                    f"     Fields joined: {result_count:,}, "
                    f"with climate data: {climate_matched:,} "
                    f"({climate_matched / result_count:.1%})"
                )

                if spatial_join_stats and spatial_join_stats[0] > 0:
                    self.log.info("   💧 PERCOLATION VALUES AFTER SPATIAL JOIN:")
                    self.log.info(
                        f"     Fields with percolation: {spatial_join_stats[1]:,}, "
                        f"positive values: {spatial_join_stats[2]:,}"
                    )
                    self.log.info(
                        f"     Total percolation: "
                        f"min={spatial_join_stats[3]:.1f}mm, "
                        f"max={spatial_join_stats[4]:.1f}mm, "
                        f"avg={spatial_join_stats[5]:.1f}mm"
                    )
                    if spatial_join_stats[6] and spatial_join_stats[5]:
                        cv_perco = spatial_join_stats[6] / spatial_join_stats[5] * 100
                        self.log.info(
                            f"     Total percolation variation: "
                            f"stddev={spatial_join_stats[6]:.1f}mm, CV={cv_perco:.1f}%"
                        )

                    self.log.info(
                        f"     April-Aug: min={spatial_join_stats[7]:.1f}mm, "
                        f"max={spatial_join_stats[8]:.1f}mm, "
                        f"avg={spatial_join_stats[9]:.1f}mm"
                    )
                    self.log.info(
                        f"     Sept-Mar: min={spatial_join_stats[10]:.1f}mm, "
                        f"max={spatial_join_stats[11]:.1f}mm, "
                        f"avg={spatial_join_stats[12]:.1f}mm"
                    )
                    self.log.info(
                        f"     Climate distance: avg={spatial_join_stats[13]:.0f}m, "
                        f"min={spatial_join_stats[14]:.0f}m, "
                        f"max={spatial_join_stats[15]:.0f}m"
                    )
                    self.log.info(
                        f"     Sufficient climate quality: "
                        f"{spatial_join_stats[16]:,}/{spatial_join_stats[0]:,} fields "
                        f"({spatial_join_stats[16] / spatial_join_stats[0]:.1%})"
                    )

                else:
                    self.log.warning(
                        f"   ⚠️ No percolation data found after spatial join "
                ...
                self.log.info(
                    f"   Year {target_year} spatial join: "
                    f"{result_count:,} fields, {climate_matched:,} with climate data"
                )
            ...
            if self.config.soil_types_dataset in loaded_tables:
                fields_complete_table = self.processor._join_with_soil_data_target_year(
                    fields_climate_table
                )
            else:
                fields_complete_table = self.processor._add_default_soil_data_target_year(
                    fields_climate_table
                )

            # Step 5: Calculate percolation effects
            self.log.info(f"   💧 Percolation effects for target year {target_year}...")
            percolation_table = self.processor._calculate_percolation_effects_target_year(
            ...
            estimates_table = self.processor._calculate_nles5_estimates_target_year(
                percolation_table, target_year
            )

            # Step 7: Validate results for this target year
            target_count = self.conn.execute(f"SELECT COUNT(*) FROM {estimates_table}").fetchone()[
                0
            ]
            if target_count == 0:
            ...
            self.log.info(
                f"   ✅ NLES5 calculations completed for target year {target_year}: "
                f"{target_count:,} estimates"
            )
    ...
    def _determine_all_target_years(self) -> List[int]:
        ...
        if self.config.target_years:
            ...
            if self.config.max_years_to_process:
                ...
                self.log.info(
                    f"🎯 Auto-selected {len(target_years)} most recent target years: {target_years}"
                )
            else:
    ...
    def _process_nles5_target_year_by_target_year_for_batch(
        ...
        try:
            ...
            # FIXED: Proper schema to prevent DECIMAL(2,1) inference
            # Note: field_uuid preserved throughout pipeline from source data
            ...
            self.conn.execute(f"""
                CREATE OR REPLACE TABLE {batch_table_name} (
                    ...
                    cvr_number VARCHAR,
                    year INTEGER,
                    area_ha DECIMAL(10,4),  -- Explicit type to prevent inference
                    crop_type VARCHAR,
                    soil_code VARCHAR,
                    soil_description VARCHAR,
                    clay_content DOUBLE,
            ...
            for target_num, target_year in enumerate(batch_years, 1):
                ...
                self.log.info(
                    f"🎯 Processing target year {target_num}/{len(batch_years)}: {target_year}"
                )
                ...
                target_estimates = self._process_single_target_year(
                    target_year, required_years, loaded_tables
                )

                # Append results to batch table
                if target_estimates:
                    target_results = self.conn.execute(
                        f"SELECT COUNT(*) FROM {target_estimates}"
                    ).fetchone()[0]
                    if target_results > 0:
                        self.conn.execute(f"""
                        ...
                        self.log.info(
                            f"   ✅ Target year {target_year}: {target_results:,} fields processed"
                        )
                    else:
            ...
            batch_count = self.conn.execute(f"SELECT COUNT(*) FROM {batch_table_name}").fetchone()[
                0
            ]
