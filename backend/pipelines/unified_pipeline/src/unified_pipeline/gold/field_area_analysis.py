        ...
        self.gcs_access.query_parquet_direct(
            ...
            f"""SELECT
                ...
                COALESCE(
                    ...
                    CAST(block_id AS VARCHAR) || '_' ||
                    CAST(field_id AS VARCHAR)
                ) as primary_field_id,
                {latest_year} as year,
                CASE
            ...
            with self.gcs_access._temp_download(self.properties_path) as temp_file:
                ...
                INSERT INTO field_property_results
                    SELECT
                        ...
                        f.cvr_number,
                        p.bfe_number,
                        ST_Area_Spheroid(ST_Intersection(f.geom, p.geom)) /
    ...
    def _generate_final_results(self) -> Dict[str, any]:
        ...
        if has_properties and has_environmental_coverage:
            self.conn.execute("""
                ...
                LEFT JOIN field_coverage_percentages ec ON (
                    COALESCE(f.field_uuid, f.primary_field_id) =
        ...
        results = {
            ...
            "coverage": {
                ...
                "water_projects": {
                    ...
                    "percentage": (
                        water_project_coverage / total_fields * 100
                        if total_fields > 0
                        else 0
                    ),
                },
                ...
                "properties": {
                    ...
                    "percentage": (
                        property_coverage / total_fields * 100
                        if total_fields > 0
                        else 0
                    ),
                },
