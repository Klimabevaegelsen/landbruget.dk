...
class DataConsolidation(BaseSource[DataConsolidationConfig], GoldJobInterface):
    ...
    def _merge_data_by_cvr(
        ...
        for cvr_number, company_data in companies_data.items():
            ...
            if cvr_number in financial_data:
                ...
                merged_company["financial_metrics"] = financial_data[cvr_number].get("financial_metrics")
            else:
    ...
    def _create_normalized_tables(self, consolidated_data: Dict[str, Any]) -> List[str]:
        ...
        """
        self.log.info("Creating sophisticated normalized CVR enrichment tables (ported from original)")
        ...
        self.log.info(f"🔍 Creating normalized tables for {len(companies_list)} companies using chunked processing")
        ...
        for table in table_names:
            try:
                ...
                if count > 0:
                    ...
                    self._save_data(
                        data=table, 
                        dataset=f"{self.config.dataset}_{table.split('_')[-1]}" if '_' in table else self.config.dataset, 
                        bucket=self.config.bucket, 
                        stage="gold"
                    )
                else:
