...
def get_straw_co2e_factor_from_table() -> float:
    ...
    for item in data.get("data", []):
        if item.get("Aktivitet") == "Samlet":
            footprint_str = (
                item.get("Klimaaftryk_pr_ton", "0.0").replace(" kg CO2e", "").replace(",", ".")
            )
            try:
                return float(footprint_str)
            except ValueError:
                print("Warning: Could not parse float for Straw CO2e factor. Using 0.0.")
                return 0.0
...
def beregn_co2e_stroelse_scope1(
    v_sp_ton: float, theta_sp1_kg_co2e_pr_ton: float, a_fjer: float
) -> float:
    """
    ...
    if a_fjer == 0:  # Prevent division by zero
        return 0.0
    return (v_sp_ton * theta_sp1_kg_co2e_pr_ton) / a_fjer


def beregn_theta_sp3_spagnum_produktion_transport(
    theta_ch4_co2: float, theta_n2o_co2: float
) -> float:
    """
    ...
    """
    return 154 + (199.0 / 1000.0) * theta_ch4_co2 + (14.0 / 1000.0) * theta_n2o_co2


def beregn_co2e_stroelse_scope3(
    v_h_ton: float,
    v_s_ton: float,
    theta_s_kg_co2e_pr_ton: float,
    v_sp_ton: float,
    theta_sp3_kg_co2e_pr_ton: float,
    a_fjer: float,
) -> float:
    """
    ...
    return (co2e_halm + co2e_spaaner + co2e_spagnum) / a_fjer
