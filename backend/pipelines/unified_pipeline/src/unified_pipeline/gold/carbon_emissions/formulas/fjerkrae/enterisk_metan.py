...
def get_enteric_ch4_factors_from_table(fjerkrae_type_key: str) -> tuple[float, float]:
    ...
    data = load_json_data(
        "tabel_8_faktorer_til_beregning_af_metan_emission_fra_fordøjelse_hos_fjerkræ_kg_ch4_per_100_eller_100.json"
    )
    default_ef = 0.0
    ...
    for item in data.get("data", []):
        if item.get("Fjerkrætype_Dage") == fjerkrae_type_key:
            ch4_ef_str = str(item.get("CH4_EF", "0.0")).replace(
                "*", ""
            ...
            try:
                ...
                print(
                    f"Warning: Could not parse float for CH4_EF '{ch4_ef_str}' for {fjerkrae_type_key}. Using defaults."
                )
                return default_ef, default_lambda

    print(
        f"Warning: Fjerkrætype_Dage '{fjerkrae_type_key}' not found in tabel_8. Using defaults for CH4_EF and lambda."
    )
    return default_ef, default_lambda
...
def beregn_ch4_enterisk_fjerkrae(fjerkrae_type_key: str) -> float:
    ...
    if lambda_fjer_table == 0:
        print(
            f"Warning: lambda_fjer is 0 for {fjerkrae_type_key}, cannot calculate CH4. Returning 0."
        )
        return 0.0

    return e_ch4_table / lambda_fjer_table  # EF is per lambda_fjer animals
...
def beregn_co2e_enterisk_fjerkrae(ch4_enterisk_pr_individ: float, theta_ch4_co2: float) -> float:
    ...
    """
    return ch4_enterisk_pr_individ * theta_ch4_co2
