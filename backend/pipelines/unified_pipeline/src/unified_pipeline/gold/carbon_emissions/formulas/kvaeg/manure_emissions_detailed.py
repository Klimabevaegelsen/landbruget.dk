...
def get_stable_emission_factors(stable_type_key: str, manure_system_type: str) -> dict[str, float]:
    """
    Fetches emission factors for a given stable type from a table (e.g., Table 11).
    ...
    if (
        stable_type_key == "Sengestald med spalter (kanal, bagskyl eller ringkanal)"
        and manure_system_type == "gylle"
    ):
        return {
            "nh3_tan_percent": 13.5,
            "n2o_total_n_percent": 0.2,
        }  # NH3 % of TAN, N2O % of Total N
    if stable_type_key == "Dybstrøelse" and manure_system_type == "dybstrøelse":
        return {
            "nh3_total_n_percent": 6.0,
            "n2o_total_n_percent": 1.0,
        }  # NH3 % of Total N for deep litter, N2O % of Total N
    # Add more stable types and gylle/dybstrøelse differentiation based on Table 11
    print(
        f"Warning: Using placeholder/default EFs for stable {stable_type_key}, {manure_system_type}"
    )
...
def calculate_stable_nh3_kg_n_pr_dyr_pr_aar(
    ...
    if manure_system_type == "gylle":
        ...
        nh3_n_kg = kg_total_n_in_stable_pr_dyr_pr_aar * (
            ef_data.get("nh3_total_n_percent", 0.0) / 100.0
        )
    else:
...
def calculate_indirect_co2e_from_nh3_kg(nh3_n_kg: float, theta_n2o_co2: float) -> float:
    """
    ...
    return n2o_mass_from_nh3_kg * theta_n2o_co2
...
def calculate_stable_direct_n2o_kg_pr_dyr_pr_aar(
    ...
    n2o_n_kg = kg_total_n_in_stable_pr_dyr_pr_aar * (
        ef_data.get("n2o_total_n_percent", 0.0) / 100.0
    )
    return n2o_n_kg * MOL_WEIGHT_N2O_N_FACTOR  # Convert N2O-N to N2O mass
...
def calculate_stable_ch4_kg_pr_dyr_pr_aar(
    ...
    b0 = methane_efs["b0"]
    mcf_stable_percent = methane_efs[
        "mcf_percent"
    ]  # Assuming this MCF is for the combined stable/storage, and split by days

    # Daily VS input
    vs_total_daily_kg = (vs_goedning_kg_pr_dyr_pr_aar / 365.0) + (
        vs_stroelse_kg_pr_dyr_pr_aar / 365.0
    )
    ...
    return (
        vs_total_daily_kg
        * days_in_stable_for_methane_calc
        * b0
        * (mcf_stable_percent / 100.0)
        * 0.67
    )
...
def calculate_manure_ab_stald(
    ...
    n_ab_stald_kg = (
        initial_total_n_pr_dyr_pr_aar - stable_direct_n2o_n_loss_kg - stable_nh3_n_loss_kg
    )
...
def get_storage_emission_factors(
    storage_type_key: str, manure_system_type: str
) -> dict[str, float]:
    """
    ...
    print(
        f"Warning: Using placeholder/default EFs for storage {storage_type_key}, {manure_system_type}"
    )
    if manure_system_type == "gylle":
        return {
            "nh3_tan_percent_storage": 5.0,
            "n2o_total_n_percent_storage": 0.5,
        }  # Example values
    if manure_system_type == "dybstrøelse":
        return {
            "nh3_total_n_percent_storage": 2.0,
            "n2o_total_n_percent_storage": 0.5,
            "proportion_stored_factor": 0.35,
        }
    return {
        "nh3_tan_percent_storage": 0.0,
        "nh3_total_n_percent_storage": 0.0,
        "n2o_total_n_percent_storage": 0.0,
        "proportion_stored_factor": 1.0,
    }
...
def calculate_storage_nh3_kg_n_pr_dyr_pr_aar(
    ...
    if manure_system_type == "gylle":
        ...
        nh3_n_kg = (
            total_n_ab_stald_kg_pr_dyr
            * (ef_data.get("nh3_total_n_percent_storage", 0.0) / 100.0)
            * proportion_factor
        )
    else:
...
def calculate_storage_direct_n2o_kg_pr_dyr_pr_aar(
    ...
    if manure_system_type == "gylle":
        ...
        n2o_n_kg = total_n_ab_stald_kg_pr_dyr * (
            ef_data.get("n2o_total_n_percent_storage", 0.0) / 100.0
        )
    elif manure_system_type == "dybstrøelse":
...
def calculate_storage_ch4_kg_pr_dyr_pr_aar(
    ...
    return (
        vs_total_daily_kg
        * days_in_storage_for_methane_calc
        * b0
        * (mcf_storage_percent / 100.0)
        * 0.67
    )


# Global GWP factors (IPCC AR6, 2021)
THETA_N2O_CO2_GLOBAL = 273.0  # GWP for N2O (IPCC AR6, 2021)
THETA_CH4_CO2_GLOBAL = 27.0  # GWP for CH4 (IPCC AR6, 2021 - biogenic methane)
