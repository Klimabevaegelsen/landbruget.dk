...
class FieldsBNBOWaterCoverage(FieldAnalysisStageBase):
    ...
    async def _execute_stage_processing(self) -> Dict[str, Any]:
        ...
        self.conn.execute("""
            ...
            SELECT
                ...
                fbi.status_category,
                wpbi.project_id,
                ST_Intersection(fbi.field_bnbo_geometry,
                               wpbi.intersection_geometry) as field_bnbo_water_geometry
    ...
    def _validate_geometry_quality(self):
        ...
        total_bnbo = self.conn.execute("SELECT COUNT(*) FROM field_bnbo_intersections").fetchone()[
            0
        ]
        ...
        if invalid_bnbo > 0:
            self.log.error(
                f"❌ Found {invalid_bnbo:,}/{total_bnbo:,} invalid/NULL geometries "
                f"in field_bnbo_intersections"
            )
        else:
        ...
        if invalid_water > 0:
            self.log.error(
                ...
                f"in field_bnbo_water_intersections"
            )
        else:
            self.log.info(
                f"✅ All {total_water:,} field_bnbo_water_intersections geometries are valid"
            )

        if empty_bnbo > 0:
            self.log.warning(
                f"⚠️ Found {empty_bnbo:,} empty geometries in field_bnbo_intersections"
            )
        if empty_water > 0:
            self.log.warning(
                f"⚠️ Found {empty_water:,} empty geometries in field_bnbo_water_intersections"
            )
    ...
    def _validate_record_counts(self):
        ...
        if field_bnbo_count == 0:
            ...
            self.log.warning(
                f"⚠️ Very high intersection count: {field_bnbo_count:,} (check for data explosion)"
            )
    ...
    def _validate_data_consistency(self):
        ...
        if water_only_fields > 0:
            self.log.error(
                f"❌ CRITICAL: {water_only_fields:,} fields have water-covered BNBO "
                f"but no total BNBO (data inconsistency)"
            )
        else:
            self.log.info(
                "✅ Data consistency: All water-covered BNBO fields also have total BNBO records"
            )
        ...
        if invalid_field_uuids > 0:
            self.log.error(
                f"❌ CRITICAL: {invalid_field_uuids:,} intersection records "
                f"have invalid field_uuid references"
            )
        else:
