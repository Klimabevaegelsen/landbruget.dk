...
class FieldAreaAnalysisConfig(BaseModel):
    ...
    agricultural_fields_year: int = int(
        os.getenv("AGRICULTURAL_FIELDS_YEAR", "2024")
    )
    ...
    wetlands_dataset: str = (
        "wetlands_dissolved"  # Use dissolved wetlands - now preserves
    ...
    properties_filtered_dataset: str = (
        "stage0_properties_filtered"  # 6.5M → ~500K (90% reduction)
    )
    bnbo_filtered_dataset: str = (
        "stage0_bnbo_filtered"  # 3.7K → ~1K (70% reduction)
    )
    wetlands_filtered_dataset: str = (
        "stage0_wetlands_filtered"  # 1.6M → ~200K (85% reduction)
    )
    water_projects_filtered_dataset: str = (
        "stage0_water_projects_filtered"  # 2.4K → ~500 (80% reduction)
    )
    soil_types_filtered_dataset: str = (
        "stage0_soil_types_filtered"  # 13K → ~8K (40% reduction)
    )

    # Processing parameters optimized for GitHub Actions with Stage 0
    # pre-filtering
    # Batch sizes can be larger due to dramatically reduced probe sizes
    batch_size: int = (
        500000  # Stage 1: Pre-filtered datasets (13x reduction in complexity)
    )
    stage2_batch_size: int = (
        25000  # Stage 2: Field-level joins with pre-filtered data (2.5x larger)
    )
    stage3_batch_size: int = (
        10000  # Stage 3: Property-level analysis with pre-filtered data
               # (2x larger)
    )
    max_memory_gb: int = 14  # Leave 2GB for system overhead
    max_threads: int = 4
    max_temp_directory_size: str = (
        "12GB"  # Leave space for downloads
    )
    ...
    memory_cleanup_frequency: int = (
        3  # Clean up every N batches for Stage 3
    )
    stage1_memory_cleanup_frequency: int = (
        5  # Less frequent for Stage 1 (simpler queries)
    )
    stage2_memory_cleanup_frequency: int = (
        2  # More frequent for Stage 2 (spatial joins)
    )

    # Output dataset names for intermediate stages
    # Note: These will be dynamically updated with year suffix via
    # update_outputs_for_year()
    stage_outputs: Dict[str, str] = {
        # Stage 0 outputs (pre-filtering for massive performance
        # improvement)
        ...
        "bnbo_water_coverage": (
            "field_analysis_bnbo_water_coverage"
        ),
        "water_projects_bnbo_intersections": (
            "field_analysis_water_projects_bnbo_intersections"
        ),
        "wetland_water_coverage": (
            "field_analysis_wetland_water_coverage"
        ),
        "water_projects_wetlands_intersections": (
            "field_analysis_water_projects_wetlands_intersections"
        ),
        "field_property_intersections": (
            "field_analysis_property_intersections"
        ),
        "field_soil_intersections": (
            "field_analysis_soil_intersections"
        ),
        # Stage 2 outputs (field-level intersection geometries only -
        # aggregations moved to Stage 4)
        "field_bnbo_intersections": (
            "field_analysis_field_bnbo_intersections"  # For Stage 3
                                                         # optimization
        ),
        "field_bnbo_water_intersections": (
            "field_analysis_field_bnbo_water_intersections"  # For Stage 3
                                                              # optimization
        ),
        "field_wetland_intersections": (
            "field_analysis_field_wetland_intersections"  # For Stage 3
                                                            # optimization
        ),
        "field_wetland_water_intersections": (
            "field_analysis_field_wetland_water_intersections"  # For Stage 3
                                                                  # optimization
        ),
        ...
        "property_bnbo_intersections": (
            "field_analysis_property_bnbo_intersections"  # For two-table
                                                            # architecture
        ),
        "property_bnbo_water_intersections": (
            "field_analysis_property_bnbo_water_intersections"  # For two-table
                                                                  # architecture
        ),
        "property_wetland_intersections": (
            "field_analysis_property_wetland_intersections"  # For two-table
                                                               # architecture
        ),
        "property_wetland_water_intersections": (
            "field_analysis_property_wetland_water_intersections"  # For
                                                                     # two-table
                                                                     # architecture
        ),
        ...
        "field_environmental_analysis": (
            "field_environmental"  # Table 1: Field-level (legacy)
        ),
        "field_environmental_analysis_fields": (
            "field_environmental_analysis_fields"  # Table 1: Field-level
                                                    # (redesigned)
        ),
        ...
        "field_environmental_analysis_properties": (
            "field_environmental_analysis_properties"  # Table 2:
                                                        # Property-level
                                                        # (redesigned)
        ),
    }
    ...
    parquet_compression: str = (
        "zstd"  # Best balance of speed/size
    )
    ...
    def update_outputs_for_year(self) -> Dict[str, str]:
        ...
        for key, dataset_name in self.stage_outputs.items():
            # Add year suffix to field analysis outputs AND stage0 outputs
            # for isolation
            # Match "field_analysis", "field_environmental_analysis", and
            # "stage0" patterns
            if (
                "field_analysis" in dataset_name
                or "field_environmental_analysis" in dataset_name
            ) or "stage0" in dataset_name:
                updated_outputs[key] = (
                    f"{dataset_name}_{self.agricultural_fields_year}"
                )
            else:
