...
class WaterProjectsBNBOIntersection(FieldAnalysisStageBase):
    ...
    def _load_input_data(self):
        """Load BNBO and water projects datasets."""
        # Load BNBO status (small dataset ~3.7K polygons)
        self._load_silver_dataset(CONFIG.bnbo_status_dataset, "bnbo_status_raw")

        # Load water projects (very small dataset ~100s polygons)
        self._load_silver_dataset(CONFIG.water_projects_dataset, "water_projects_raw")

        # Use ST_Dump for multipolygon decomposition and better spatial indexing
        self.log.info("Decomposing BNBO polygons with ST_Dump for optimal spatial performance...")
