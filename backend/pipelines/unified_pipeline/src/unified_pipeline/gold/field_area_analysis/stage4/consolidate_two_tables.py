        ...
        self.conn.execute("""
            ...
            SELECT
                ...
                CASE
                    ...
                    THEN (
                        -- Cap BNBO area to field area if difference is small
                        -- (≤0.1 m²) to handle spatial precision errors
                        LEAST(
                ...
                CASE
                    WHEN COALESCE(ba.field_bnbo_total_m2, 0) > 0
                    THEN (
                        -- Cap water area to BNBO area if difference is small
                        -- (≤0.1 m²) to handle spatial precision errors
                        LEAST(
                ...
                COALESCE(bwa.bnbo_action_required_water_covered_hectares, 0)
                    as bnbo_action_required_water_covered_hectares,
                COALESCE(bwa.bnbo_completed_water_covered_hectares, 0)
                    as bnbo_completed_water_covered_hectares,
                ...
                CASE
                    ...
                    THEN (
                        -- Cap wetland area to field area if difference is small
                        -- (≤0.1 m²) to handle spatial precision errors
                        LEAST(
                ...
                CASE
                    WHEN COALESCE(wa.field_wetland_total_m2, 0) > 0
                    THEN (
                        -- Cap water area to wetland area if difference is small
                        -- (≤0.1 m²) to handle spatial precision errors
                        -- This preserves legitimate high coverage while fixing
                        -- tiny calculation discrepancies
                        LEAST(
