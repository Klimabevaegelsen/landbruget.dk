...
class FinalWetlandAnalysis(FieldAnalysisStageBase):
    ...
    def _load_input_data(self):
        ...
        self.log.info(
            f"✅ Loaded field_wetland_intersections from {stage2b_wetland_dataset}"
        )
        ...
        self.log.info(
            f"✅ Loaded field_wetland_water_intersections from {stage2b_water_dataset}"
        )

        self.log.info(
            "✅ SIMPLIFIED STAGE 3B: Using existing geometries from Stage 2B"
        )
        ...
        self.log.info(
            f"  Field-wetland-water intersections: {water_wetland_count:,}"
        )
    ...
    async def _execute_stage_processing(self) -> Dict[str, Any]:
        ...
        geometries:
        1. property_wetland_intersections: Properties × Wetland total
           geometries
        2. property_wetland_water_intersections: Properties × Wetland
        ...
        - SPATIAL_JOIN Compliance: Single spatial predicates in JOIN ON
          clauses
        - Use existing geometries from Stage 2B: No complex spatial
          recalculations
        """

        self.log.info(
            "🎯 SIMPLIFIED STAGE 3B: PROPERTY-LEVEL WETLAND GEOMETRIES"
        )
        self.log.info(
            "✅ GEOMETRY ONLY - All area calculations moved to Stage 4"
        )
        self.log.info(
            "🔧 Using existing geometries from Stage 2B - No complex recalculations"
        )

        # Step 1: Create property × wetland intersections (total wetlands
        # within properties)
        ...
        self.conn.execute("""
            ...
            SELECT
                ...
                fwi.toerv_pct,
                ST_Intersection(
                    p.property_geometry, fwi.field_wetland_geometry
                ) as property_wetland_geometry
        ...
        self.conn.execute("""
            ...
            SELECT
                ...
                fwwi.project_id,
                ST_Intersection(
                    p.property_geometry, fwwi.field_wetland_water_geometry
                ) as property_wetland_water_geometry
            FROM field_property_intersections p
            JOIN stage3b_field_wetland_water_intersections fwwi
                ON p.field_uuid = fwwi.field_uuid
                AND ST_Intersects(
                    p.property_geometry, fwwi.field_wetland_water_geometry
                )
        """)
        ...
        self.log.info(
            f"📊 Property-wetland intersections: {property_wetland_count:,}"
        )
        self.log.info(
            f"📊 Property-wetland-water intersections: {property_wetland_water_count:,}"
        )
        self.log.info(
            f"📊 Unique properties with wetlands: {unique_properties:,}"
        )
        self.log.info(
    ...
    def _save_output_data(self, result: Dict[str, Any]):
        ...
        self._save_stage_output(
            "property_wetland_intersections", "property_wetland_intersections"
        )
    ...
    def _validate_geometric_output(self):
        ...
        self.log.info(
            "🔍 REDESIGNED STAGE 3B: Running comprehensive validations..."
        )
        ...
        self.log.info(
            "✅ REDESIGNED STAGE 3B: All comprehensive validations completed"
        )
    ...
    def _validate_geometry_quality(self):
        ...
        invalid_wetland = self.conn.execute("""
            ...
            WHERE property_wetland_geometry IS NULL
               OR NOT ST_IsValid(property_wetland_geometry)
        """).fetchone()[0]
        ...
        invalid_water = self.conn.execute("""
            ...
            WHERE property_wetland_water_geometry IS NULL
               OR NOT ST_IsValid(property_wetland_water_geometry)
        """).fetchone()[0]
        ...
        empty_wetland = self.conn.execute("""
            SELECT COUNT(*)
            FROM property_wetland_intersections
            WHERE property_wetland_geometry IS NOT NULL
              AND ST_IsEmpty(property_wetland_geometry)
        """).fetchone()[0]

        empty_water = self.conn.execute("""
            SELECT COUNT(*)
            FROM property_wetland_water_intersections
            WHERE property_wetland_water_geometry IS NOT NULL
              AND ST_IsEmpty(property_wetland_water_geometry)
        """).fetchone()[0]
        ...
        if invalid_wetland > 0:
            self.log.error(
                f"❌ Found {invalid_wetland:,}/{total_wetland:,} invalid/NULL "
                f"geometries in property_wetland_intersections"
            )
        else:
            self.log.info(
                f"✅ All {total_wetland:,} property_wetland_intersections "
        ...
        if invalid_water > 0:
            self.log.error(
                ...
                f"geometries in property_wetland_water_intersections"
            )
        else:
            self.log.info(
                f"✅ All {total_water:,} property_wetland_water_intersections "
                f"geometries are valid"
            )

        if empty_wetland > 0:
            self.log.warning(
                f"⚠️ Found {empty_wetland:,} empty geometries in "
    ...
    def _validate_record_counts(self):
        ...
        self.log.info(
            f"📊 Output data: {property_wetland_count:,} property×wetland "
            f"intersections, {property_wetland_water_count:,} "
            f"property×wetland×water intersections"
        )
        self.log.info(
            f"📊 Property coverage: {unique_properties_with_wetland:,} properties "
            f"with wetlands, {unique_properties_with_water:,} properties with "
            f"water-covered wetlands"
        )

        # Sanity checks
        if property_wetland_count == 0:
            self.log.warning(
                "⚠️ No property×wetland intersections produced (may be expected "
                "if no field×property×wetland overlap)"
            )
    ...
    def _validate_data_consistency(self):
        ...
        # Validate that water-covered wetland properties are subset of total
        # wetlands
        water_only_properties = self.conn.execute("""
            SELECT COUNT(DISTINCT pww.bfe_number)
            FROM property_wetland_water_intersections pww
            LEFT JOIN property_wetland_intersections pw
                ON pww.bfe_number = pw.bfe_number
                AND pww.field_uuid = pw.field_uuid
        ...
        if water_only_properties > 0:
            self.log.error(
                ...
                f"water-covered wetlands but no total wetland (data inconsistency)"
            )
        else:
            self.log.info(
                "✅ Data consistency: All water-covered wetland properties also "
                "have total wetland records"
            )
        ...
        invalid_bfe_numbers = self.conn.execute("""
            ...
            LEFT JOIN field_property_intersections fp
                ON pw.field_uuid = fp.field_uuid
                AND pw.bfe_number = fp.bfe_number
        ...
        if duplicates > 0:
            self.log.error(
                f"❌ CRITICAL: {duplicates:,} duplicate field×property×wetland "
                f"combinations found"
            )
        else:
            self.log.info(
                "✅ No duplicate field×property×wetland combinations found"
            )
