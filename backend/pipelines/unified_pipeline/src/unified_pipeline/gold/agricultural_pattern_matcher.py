...
class AgriculturalPatternMatcher(BaseSource[AgriculturalPatternMatcherConfig], GoldJobInterface):
    ...
    def __init__(
        self, config: AgriculturalPatternMatcherConfig, db_connection=None, fvm_table_name="marker"
    ):
    ...
    def _initialize_table_names(self) -> None:
        ...
        self.table_names = {
            ...
            "gkea_agricultural_signatures": self._get_table_name(
                "gkea_agricultural_signatures", temporary=True
            ),
            "gkea_crop_profiles": self._get_table_name("gkea_crop_profiles", temporary=True),
            "fvm_agricultural_signatures": self._get_table_name(
                "fvm_agricultural_signatures", temporary=True
            ),
            "fvm_crop_profiles": self._get_table_name("fvm_crop_profiles", temporary=True),
            "agricultural_pattern_matches": self._get_table_name(
                "agricultural_pattern_matches", temporary=True
            ),
            "agricultural_field_mappings": self._get_table_name(
                "agricultural_field_mappings", temporary=True
            ),
            "enhanced_gkea_fvm_matches": self._get_table_name(
                "enhanced_gkea_fvm_matches", temporary=False
            ),  # Persistent
            "agricultural_matching_summary": self._get_table_name(
                "agricultural_matching_summary", temporary=False
            ),  # Persistent
        }
    ...
    async def run(self, silver_data: dict[str, Any] | None = None) -> None:
        ...
        try:
            ...
            self.log.error(f"❌ Agricultural Pattern Matching failed: {e!s}")
            raise
        finally:
    ...
    async def _load_current_matching_state(self) -> None:
        ...
        try:
            ...
            unmatched_count = self.db.execute(
                f"SELECT COUNT(*) FROM {self.table_names['unmatched_gkea_fields']}"
            ).fetchone()[0]
    ...
    async def _create_gkea_agricultural_signatures(self) -> None:
        ...
        if self.db is None:
            self.log.warning(
                "   No database connection available - skipping GKEA signature creation"
            )
            return
        ...
        try:
            ...
            gkea_ops = self.db.execute(
                "SELECT COUNT(*) FROM gkea_agricultural_signatures"
            ).fetchone()[0]
    ...
    async def _create_fvm_agricultural_signatures(self) -> None:
        ...
        if self.db is None:
            self.log.warning(
                "   No database connection available - skipping FVM signature creation"
            )
            return
        ...
        try:
            ...
            fvm_ops = self.db.execute(
                "SELECT COUNT(*) FROM fvm_agricultural_signatures"
            ).fetchone()[0]
    ...
    async def _find_agricultural_pattern_matches(self) -> None:
        ...
        try:
            ...
            pattern_matches = self.db.execute(
                "SELECT COUNT(*) FROM agricultural_pattern_matches"
            ).fetchone()[0]
    ...
    async def _create_field_mappings_from_patterns(self) -> None:
        ...
        try:
            ...
            self.matches_found = self.db.execute(
                "SELECT COUNT(*) FROM agricultural_field_mappings"
            ).fetchone()[0]
            ...
            if self.matches_found > 0:
                ...
                self.log.info(
                    f"   Mapping quality: Pattern {stats[0]:.3f}, Field {stats[1]:.3f}, "
    ...
    async def _validate_and_store_matches(self) -> None:
        ...
        try:
            ...
            original_unmatched = self.db.execute(
                f"SELECT COUNT(*) FROM {self.table_names['unmatched_gkea_fields']}"
            ).fetchone()[0]
            improvement_rate = (
                self.matches_found / original_unmatched if original_unmatched > 0 else 0
            )

            self.log.info(
                f"   Validation complete: {self.matches_found:,} high-quality matches stored"
            )
...
async def run_agricultural_pattern_matching(
    config: AgriculturalPatternMatcherConfig | None = None,
    db_connection=None,
