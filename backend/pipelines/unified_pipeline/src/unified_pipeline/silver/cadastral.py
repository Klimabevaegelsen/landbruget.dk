...
class CadastralSilver(BaseSource[CadastralSilverConfig], SilverJobInterface):
    ...
    def _validate_and_transform_from_table(self, source_table_name: str) -> str:
        ...
        try:
            ...
            geometry_condition = (
                "geometry IS NOT NULL AND geometry != ''" if "geometry" in column_names else "FALSE"
            )
            ...
            final_count = self.conn.execute(f"SELECT COUNT(*) FROM {target_table_name}").fetchone()[
                0
            ]
