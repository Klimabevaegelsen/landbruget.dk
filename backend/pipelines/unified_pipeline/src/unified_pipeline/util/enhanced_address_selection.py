...
class EnhancedAddressSelector:
    ...
    def _filter_candidates(self, addresses: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # Filter by current status if required, but fallback to most recent historical
        # ONLY if no current addresses exist
        if self.config.prefer_current_only:
            current_addresses = [addr for addr in candidates if addr.get("is_current")]
            if current_addresses:
                # Company has current addresses - use only current addresses (existing behavior)
                candidates = current_addresses
            else:
                # Company has NO current addresses - fallback to most recent historical
                # addresses for analytical purposes
    ...
    def _select_first_geocoded(self, addresses: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            First address that is current and geocoded, or most recent historical geocoded
            address (only if no current addresses), or None
        """
        # First try current and geocoded addresses
        current_geocoded = [
            addr for addr in addresses if addr.get("is_current") and addr.get("dawa_enriched")
        ]
        if current_geocoded:
            return current_geocoded[0]

        # Check if company has ANY current addresses at all
        current_addresses = [addr for addr in addresses if addr.get("is_current")]
        if current_addresses:
            # Company has current addresses but none are geocoded - don't fallback to historical
            return None

        # Company has NO current addresses - fallback to most recent historical geocoded
        # addresses for analytical purposes
        historical_geocoded = [
            addr for addr in addresses if not addr.get("is_current") and addr.get("dawa_enriched")
        ]
    ...
    def _select_by_quality(self, addresses: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        candidates.sort(
            key=lambda addr: (
                self.config.coordinate_quality_priority.get(
    ...
    def _select_hybrid(self, addresses: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        def hybrid_score(addr: Dict[str, Any]) -> tuple:
            ...
            type_priority = self.config.address_type_priority.get(addr.get("address_type", ""), 99)
    ...
    def analyze_selection_options(self, addresses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        analysis = {"total_addresses": len(addresses), "strategy_results": {}}
