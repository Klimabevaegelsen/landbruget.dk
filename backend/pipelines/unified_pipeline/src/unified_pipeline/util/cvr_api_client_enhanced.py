    ...
    def enrich_company_with_geometry(self, company_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            for address in addresses:
                                ...
                                enriched_address.update(
                                    {
                                        ...
                                        "geometry_geojson": (
                                            self.dawa_client.create_geometry_geojson(
                                                geocoded["latitude"], geocoded["longitude"]
                                            )
                                        ),
                                        "coordinate_quality": geocoded.get(
                                            "coordinate_quality", "C"
                                        ),
                ...
                if not enriched_address.get("dawa_enriched") and not enriched_address.get(
                    "datavask_enriched"
                ):
            ...
            primary_geometry = get_primary_address_geometry_enhanced(
                enriched_addresses, strategy=self.address_selection_strategy
            )
            ...
            if primary_geometry:
                ...
                if self.log.level <= 10:  # DEBUG level
                    ...
                    for addr in enriched_addresses:
                        if addr.get("latitude") == primary_geometry.get("latitude") and addr.get(
                            "longitude"
                        ) == primary_geometry.get("longitude"):
                            selected_addr = addr
                            break
...
def create_enhanced_cvr_client(
    address_selection_strategy: str = "hybrid", **kwargs
    ...
    """
    return EnhancedCVRAPIClient(address_selection_strategy=address_selection_strategy, **kwargs)
