...
Second, the FVM Marker dataset, published by Landbrugsstyrelsen (the Danish Agricultural Agency), provides annual georeferenced field boundaries and crop classifications [Landbrugsstyrelsen, 2024]. The public early-year Marker layers do not themselves contain complete field-level CVR values; for field years 2005, 2007, and 2008--2014, those values are completed from a separate historical Fællesskema/IMK administrative extract.
...
Each processed FVM record includes a unique field identifier, a deterministic UUID derived from field geometry, a crop code from the standardized Fællesskema classification, the field area in hectares calculated from the registered polygon geometry, an organic farming indicator, the geometry as a polygon in EPSG:25832 (UTM zone 32N, the standard Danish projection), a block identifier, and a municipality code. CVR is natively available from 2016 onward; the documented historical extract completes it only for 2005, 2007, and 2008--2014.

The dataset contains approximately 600,000 to 740,000 field records per year. Where CVR is native or completed from the documented historical archive, the SJI and processed FVM Marker records use the same CVR identifier and Fællesskema crop code classification, which forms the basis of the deterministic matching approach. The use of a common administrative identifier --- rather than probabilistic spatial or statistical linkage --- distinguishes this method from all prior approaches reviewed in Section 1.2.
...
The data sources are Danish government administrative data. Public WFS sources provide the FVM geometries and published attributes; the early-year field-CVR archive was received through *aktindsigt* (its workbooks do not retain a request reference). Table 1 summarizes their characteristics.
...
| FVM Marker + historical CVR extract | Landbrugsstyrelsen | GeoJSON (WFS) + SAS-generated Excel extract | 2005, 2007, 2008--2024 | Individual field | field_id, crop_code, area_ha, geometry; CVR from WFS from 2016 and the historical extract for 2005, 2007, 2008--2014 | WFS public; early CVR extract received via *aktindsigt* |
