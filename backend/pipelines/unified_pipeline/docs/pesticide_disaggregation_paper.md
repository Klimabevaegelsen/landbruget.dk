...
The method links identifiable companies (CVR) to specific pesticide applications on georeferenced fields. In Danish agriculture, a CVR frequently corresponds to an individual farmer or family operation. This creates tension between transparency and data protection. Denmark is subject to GDPR, and whether CVR-linked pesticide application profiles constitute personal data for sole proprietorships has legal implications. The SJI data was obtained through FOI requests; FOI access does not necessarily authorize public redistribution of disaggregated, individually identifiable records. California's PUR system, by comparison, excludes grower identity from public data releases. The landbruget.dk platform applies aggregation and does not expose individual CVR-level application records in its public-facing interfaces. Any replication of this approach in other jurisdictions must consider local data protection frameworks and obtain appropriate ethical review.
