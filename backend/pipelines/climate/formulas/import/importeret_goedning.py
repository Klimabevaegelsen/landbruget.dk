...
def get_fertilizer_constants_from_table():
    ...
    for item in data.get("data", []):
        ...
        footprint_str = (
            item.get("Carbon_Footprint_CO2_aekv_per_kg", "0.0").split(" ")[0].replace(",", ".")
        )
        try:
            value = float(footprint_str)
        except ValueError:
            print(
                f"Warning: Could not parse float from '{footprint_str}' for {mineral}. Using 0.0."
            )
            value = 0.0
        ...
        if "Kvælstof (N)" in mineral:
            ...
            constants["kk"] = value
    return (
        constants.get("nk", 0.0),
        constants.get("pk", 0.0),
        constants.get("kk", 0.0),
...
def beregn_co2e_importeret_goedning(
    n_total: float, p_total: float, k_total: float, areal: float
) -> float:
    """
    ...
    return (n_total * NK_KONSTANT + p_total * PK_KONSTANT + k_total * KK_KONSTANT) * areal
