...
THETA_CH4_CO2 = 27.0  # GWP for CH4 (IPCC AR6, 2021 - biogenic methane)
...
def calculate_ch4_enteric_malkeko_tung_race_pr_aar(
    foderoptag_kg_ts_pr_dag: float, fedtsyre_g_pr_kg_ts: float, ndf_g_pr_kg_ts: float
) -> float:
    """
    ...
    """
    ch4_malkende_periode = (
        (1.230 * foderoptag_kg_ts_pr_dag - 0.145 * fedtsyre_g_pr_kg_ts + 0.012 * ndf_g_pr_kg_ts)
        / 55.65
        * 335.0
    )
...
def calculate_ch4_enteric_opdraet_og_tyre_aeldre_pr_aar(
    ...
    return (ch4_numerator / 55.65) * 365.0
...
if __name__ == "__main__":
    ...
    print(
        f"  CO2e pr. dyr pr. periode (opdræt 0-6mdr tung): {ch4_opdr_0_6_tung * THETA_CH4_CO2:.2f} kg"
    )
