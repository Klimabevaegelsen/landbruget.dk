...
class ClimateDataLoader:
    ...
    def load_soil_types(self) -> pd.DataFrame:
        """
        ...
        Returns:
            DataFrame with soil type polygons and geometries
        """
        try:
            ...
            logger.info(f"✅ Loaded {len(result_df)} soil type polygons")
            return result_df
    ...
    def load_lavbundsjord(self, cvr: str, year: int) -> pd.DataFrame:
        """
        ...
        Returns:
            DataFrame with columns: field_id, wetland_id, toerv_pct, field_wetland_area_m2,
        ...
        try:
            ...
            columns = self.gcs.duckdb_conn.execute(sample_query).df().columns.tolist()
            ...
            logger.info(f"✅ Loaded {len(result_df)} lavbundsjord records for CVR {cvr_padded}")

            # Summary statistics
            if not result_df.empty:
                total_wetland_area_ha = result_df['area_ha'].sum()
                carbon_6_12 = result_df[result_df['carbon_pct'] == '6-12%']
                carbon_gt_12 = result_df[result_df['carbon_pct'] == '>12%']

                logger.info(f"   Total lavbundsjord area: {total_wetland_area_ha:.1f} ha")
                if len(carbon_6_12) > 0:
                    area_6_12 = carbon_6_12['area_ha'].sum()
                    logger.info(f"   6-12% carbon: {len(carbon_6_12)} intersections ({area_6_12:.1f} ha)")
                if len(carbon_gt_12) > 0:
                    area_gt_12 = carbon_gt_12['area_ha'].sum()
                    logger.info(f"   >12% carbon: {len(carbon_gt_12)} intersections ({area_gt_12:.1f} ha)")
    ...
    def load_organic_certification(self, cvr: str, year: int) -> Dict[str, bool]:
        """
        ...
        try:
            ...
            fields_df = self.load_fields(cvr, year)

            if fields_df.empty:
                logger.info(f"No fields found for CVR {cvr_padded}, cannot determine organic status")
                return {
            ...
            area_col = 'areal_ha' if 'areal_ha' in fields_df.columns else 'area_ha'
            total_area_ha = fields_df[area_col].sum()

            # Get journal numbers from fields (needed to match organic data)
            journal_numbers = []
            if 'journal_number' in fields_df.columns:
                journal_numbers = fields_df['journal_number'].dropna().unique().tolist()
            elif 'fs_journal_number' in fields_df.columns:
                journal_numbers = fields_df['fs_journal_number'].dropna().unique().tolist()
            ...
            query = f"""
                SELECT
                    ...
                    conversion_date,
                    deregistration_date,
                    ST_Area(ST_Transform(geometry, 'EPSG:4326', 'EPSG:25832')) / 10000 as area_ha
                FROM {table_name}
            ...
            organic_df = self.gcs.duckdb_conn.execute(query).df()
            ...
            # Calculate organic area
            organic_area_ha = organic_df['area_ha'].sum()
            organic_percentage = (organic_area_ha / total_area_ha * 100) if total_area_ha > 0 else 0.0
    ...
    def load_nles5_nitrogen(self, cvr: str, year: int) -> pd.DataFrame:
        """
        ...
        Returns:
            DataFrame with columns: field_id, nitrogen_washout_kg_ha, crop_type, area_ha,
        ...
        try:
            ...
            if not files:
                logger.warning(f"No NLES5 nitrogen data found")
                return pd.DataFrame()
    ...
    def load_cattle_breeds(self, cvr: str, year: int) -> pd.DataFrame:
        """
        ...
        Returns:
            DataFrame with columns: breed, animal_count
            Empty DataFrame if no CHR data available
        ...
        try:
            ...
            if not herd_owners_files:
                logger.debug(f"No CHR herd_owners data found (breed detection unavailable)")
                return pd.DataFrame()

            # Load animal movements (herd_number → breed)
            animal_movements_pattern = f"gs://{self.bucket}/silver/chr_dyr_animal_movements*.parquet"
            animal_movements_files = self.gcs.list_files(animal_movements_pattern)

            if not animal_movements_files:
                logger.debug(f"No CHR animal movements data found (breed detection unavailable)")
                return pd.DataFrame()

            # Create tables
            self.gcs.create_table_from_gcs("chr_herd_owners_temp", sorted(herd_owners_files)[-1])
            self.gcs.create_table_from_gcs("chr_animal_movements_temp", sorted(animal_movements_files)[-1])
            ...
            if not result_df.empty:
                logger.info(f"✅ Loaded breed data for CVR {cvr}: {len(result_df)} breeds found")
            else:
