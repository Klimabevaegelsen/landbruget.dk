...
def validate_gwp_factors():
    ...
    # Check GWP values (IPCC AR6, 2021)
    ...
    assert data["gwp_100"]["CH4"] == 27, "CH4 GWP-100 (AR6 biogenic) should be 27"
    assert data["gwp_100"]["N2O"] == 273, "N2O GWP-100 (AR6) should be 273"
...
def validate_loader_functions():
    ...
    assert get_gwp("N2O") == 273, "N2O GWP lookup failed"
...
def validate_conversions_module():
    ...
    # Check constants are loaded (IPCC AR6, 2021)
    assert CH4_GWP == 27, "CH4_GWP not loaded correctly"
    assert N2O_GWP == 273, "N2O_GWP not loaded correctly"
    assert abs(N2O_N_FACTOR - 1.5714) < 0.001, "N2O_N_FACTOR not loaded correctly"

    # Check conversion functions
    assert ch4_to_co2e(100) == 2700, "CH4 to CO2e conversion failed"
    assert n2o_to_co2e(10) == 2730, "N2O to CO2e conversion failed"
