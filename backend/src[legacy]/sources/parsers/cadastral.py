...
class Cadastral(GeospatialSource):
    ...
    def __init__(self, config):
        ...
        self.request_timeout_config = aiohttp.ClientTimeout(total=self.request_timeout, connect=60, sock_read=300)

        self.total_timeout_config = aiohttp.ClientTimeout(total=self.total_timeout, connect=60, sock_read=300)
    ...
    @backoff.on_exception(backoff.expo, (ClientError, asyncio.TimeoutError), max_tries=3, max_time=60)
        ...
        async with self.request_semaphore:
            ...
            try:
                ...
                async with session.get(
                    self.config["url"], params=params, timeout=timeout or self.request_timeout_config
                ) as response:
                    ...
                    try:
                        ...
                        if chunk:
                            ...
                            if len(features_batch) >= self.batch_size or is_last_batch:
                                logger.info(
                                    f"Writing batch of {len(features_batch):,} features (is_last_batch: {is_last_batch})"
                                )
