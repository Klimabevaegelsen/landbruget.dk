...
class Wetlands(GeospatialSource):
    ...
    def analyze_geometry(self, geom):
        ...
        is_grid_aligned = all(abs(round(coord / 10) * 10 - coord) < 0.01 for vertex in vertices for coord in vertex)
    ...
    def _parse_feature(self, feature):
        ...
        try:
            geom = self._parse_geometry(feature.find(".//gml:Polygon", self.namespaces))
    ...
    async def sync(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(self.config["url"], params=params) as response:
                ...
                features = [self._parse_feature(f) for f in root.findall(".//natur:kulstof2022", self.namespaces)]
    ...
    async def write_to_storage(self, features, dataset):
        ...
        try:
            ...
            if hasattr(self, "is_sync_complete") and self.is_sync_complete:
                ...
                dissolved_gdf = gpd.GeoDataFrame(geometry=merged_polygons, crs=combined_gdf.crs)
    ...
    async def _fetch_chunk(self, session, start_index):
        ...
        try:
            async with self.request_semaphore:
                ...
                async with session.get(self.config["url"], params=params, timeout=self.request_timeout) as response:
                    text = await response.text()
                    root = ET.fromstring(text)
