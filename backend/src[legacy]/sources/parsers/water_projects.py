...
class WaterProjects(GeospatialSource):
    ...
    def __init__(self, config):
        ...
        self.request_timeout_config = ClientTimeout(total=self.request_timeout, connect=60, sock_read=300)

        self.headers = {"User-Agent": "Mozilla/5.0 QGIS/33603/macOS 15.1"}
        ...
        self.service_types = {"Klima_lavbund_demarkation___offentlige_projekter:0": "arcgis"}
    ...
    def _parse_feature(self, feature, layer_name):
        ...
        try:
            ...
            geom_elem = feature.find("{%s}the_geom" % namespace) or feature.find("{%s}wkb_geometry" % namespace)
            ...
            data = {"layer_name": layer_name, "geometry": geometry_data["wkt"], "area_ha": geometry_data["area_ha"]}
    ...
    async def _fetch_chunk(self, session, layer, start_index):
        ...
        async with self.request_semaphore:
            ...
            try:
                async with session.get(url, params=params, timeout=self.request_timeout_config) as response:
                    ...
                    try:
                        ...
                        async with session.get(base_url, params=params) as response:
                            ...
                            for start_index in range(self.batch_size, total_features, self.batch_size):
                                logger.info(
                                    f"Layer {layer}: fetching features {start_index:,}-{min(start_index + self.batch_size, total_features):,} of {total_features:,}"
                                )
